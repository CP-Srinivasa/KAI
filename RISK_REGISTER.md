...
| R-PH4-013 | Opening PH4L before V-4 closeout would mix product-quality work with technical instability. | high | medium | Gate PH4L opening behind V4 dual-write/DB-primary closeout and baseline reconfirmation. | open |
| R-PH4-014 | Leaving dual-write and DB-primary half-open weakens auditability for later operational reviews. | high | medium | Finish V-4 write/read path closeout before new scope activation. | open |
...
- Technical baseline (compact status): `1590 passed`, `5 failed (DB-pre-existing)`, `ruff clean`.
