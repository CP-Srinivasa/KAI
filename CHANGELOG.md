...
**Zuerst nachgebaut, dann geloescht** — die drei Zusagen, die sonst ersatzlos verschwunden waeren: Reserve-Boden als Regel `reserve_floor` in der Payment-Regelkette (vor `amount_limits`, `APP_PAYMENT_RESERVE_FLOOR_SAT`, Default 0 = aus, unbekannte Liquiditaet bei bewaffnetem Boden = DENY; die Bilanz kommt als `RailHealth.available_balance_sat` aus dem Health-Aufruf des Rails — Kanal-Local plus On-Chain-Total wie die alte Regel — und der Service reicht sie in den `PolicyContext`); Truth-Anker `attest_payment_journal_tip` in `payments/journal_chain.py`, den `scripts/truth_anchor_run.py` statt `attest_ln_ops_tip` verankert (sonst waere keine Geldbewegung mehr on-chain); Empfangspfad nach `app/lightning/receive_gate.py` EXTRAHIERT statt migriert, weil `PaymentService.create_invoice` den anonymen `/oracle`-Mint hinter den Geldjournal-Lock schoebe (BL-2) und die Memo-Semantik `kai-oracle:{scope}` die Einnahmenzuordnung traegt.

**Geloescht:** `value_layer`, `policy`, `control_gate`, `idempotency_store`, `ops_annotations`, `ops_resolution`, `plan_guards`, `input_contract_rejections`, `ln_control_gates`, `scripts/ln_policy_set.py`, `scripts/redact_ln_ops_ledger.py`, `GET /dashboard/api/ln/ops` samt `LnOpsAuditPanel`. `ln_control` schrumpft auf zwei Aktionen (`pay_invoice` delegiert, `create_invoice` Empfangs-Gate); keysend/send_coins/open_channel/close_channel sind entfernt statt abgelehnt (ADR §1 DEFERRED). `ops_ledger.py` ist ein READ-ONLY-Archiv: es kann einen Alt-Vorgang nur noch SCHLIESSEN, nicht eroeffnen.

**Unangetastet:** `artifacts/ln_ops_ledger_v2.jsonl` (liegt am Geraet, bleibt in `DEFAULT_SOURCES` und `MONEY_SOURCES`), Reconciler, Dual-Read und `ln_reconciliation_eval` (PR 2), jede systemd-Unit.
...
## 2026-08-25 - STAB-02: Runtime-Identität — welcher Commit läuft wirklich? (/health, Health-Check, Deploy-Urteil)

Befund 25.08. (live): `kai-server` lief seit 18.08. 22:30 auf `79e6fca7`, Checkout 23 / Mainline 27 Commits weiter — vier Fast-Forwards ohne Restart, `/health` kannte nur `ok`. Neu: `app/core/runtime_identity.py` friert Commit, `requirements.lock`-Hash, Startzeit und PID **einmal beim Start** ein (Artefakt `artifacts/runtime/runtime_identity.json`, atomar); `/health` liefert `runtime_commit`, `checkout_commit`, `drift_commits`, `started_at_utc`, `uptime_s`, `lock_changed` (fail-soft, `None` = nicht messbar ≠ aktuell; Checkout wird ohne Subprozess aus `.git` gelesen, auch in Worktrees/packed-refs, `git rev-list` nur bei bewegtem Checkout). Health-Check: `_check_runtime_identity` meldet Drift, wenn der Checkout länger als 60 min auf dem neuen Commit steht (warning; > 24 h critical), fehlendes Artefakt auf der Pi (warning) und geänderte Lock-Datei (warning, `pip install -e .`). Deploy: `pi_deploy_step.sh` liest jetzt den `/health`-Body; `pi_deploy_runtime_reason` ergibt `RUNTIME_DRIFT_AFTER_RESTART:<n>` (FAILED — Restart hat den Code nicht geladen), `RUNTIME_STALE_NO_RESTART:<n>` (HOLD) oder `RUNTIME_IDENTITY_UNKNOWN` (HOLD, alter Server). Eine Bewertungsfunktion für alle Konsumenten (keine doppelten Invarianten). Kein Restart in diesem PR — die Felder erscheinen auf der Pi erst nach dem Deploy-Fenster STAB-07.
...
- **`README.md` / `ARCHITECTURE.md`**: stale Zustands-Header (2026-05-29 / 2026-05-21) ersetzt; Zielbild als Vision+Wegpunkt neu gerahmt; was heute real läuft (Wahrheitskette live, Paper-Engine als Messinstrument, LN-Wert-Schicht policy-gegated). Alpha-Ära-Schichten (`app/signals`-Generatoren, `app/execution`, `app/alerts`, `app/observability`) explizit als **„Messinstrument des Labors — feature-eingefroren, nur Pflege"** beschildert (B1).
- **`SECURITY.md` (NEU, kanonisch, Top-Level)**: Threat-Model-Kurzfassung (fail-closed by default, Triple-Flag+ACK-Sentinel für Live, HOTP-Kapital-Confirm, invertierte LN-Auth-Allowlist, CF-Tunnel-only-Ingress, 127.0.0.1-Bind, systemd-Hardening), Audit-/Attestation-Kette (Hash-Chain + OTS), Secrets-Handling, Index der 11 Specs unter `docs/security/`. Keine Live-Werte/Exploit-Details; offene Härtungen nur generisch.
- **`RISK_REGISTER.md` / `ASSUMPTIONS.md` (reaktiviert, lebend, Top-Level)**: echte heutige Risiken R1–R7 (Bus-Faktor 1, Nachfrage unbewiesen, Ein-Maschinen-Pi, LN-Wert-Schicht armiert, God-File-Zone, Flag-Komplexität, Supply-Chain) und aktive Annahmen AS-1..AS-4 mit Status; Archiv-Stände bleiben unverändert unter `docs/archive/`.
- **ADR 0012 — `## Addendum (2026-07-02)`** (Historie unangetastet): (a) Wegpunkt-innerhalb-Vision-Klarstellung; (b) Demand-Korrektur — nie fair getestet, unbewiesen statt widerlegt (G0-Pfad war gated/ungelistet); (c) Sunk-Cost-Formulierung durch Grenzkosten-Argument superseded; (d) drei billige Alternativen formal bewertet (OSS-Extraktion / LN-Fokus / Lernprojekt) mit M3-Revisit-Trigger (~2026-09-29).
- **`docs/adr/README.md` (NEU)**: ADR-Index 0001–0013 mit Status + Nummern-Hygiene (0006 doppelt vergeben, 0008 nie vergeben; keine rückwirkende Umnummerierung — Verweis-Stabilität).
