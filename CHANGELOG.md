...
**Zuerst nachgebaut, dann geloescht** — die drei Zusagen, die sonst ersatzlos verschwunden waeren: Reserve-Boden als Regel `reserve_floor` in der Payment-Regelkette (vor `amount_limits`, `APP_PAYMENT_RESERVE_FLOOR_SAT`, Default 0 = aus, unbekannte Liquiditaet bei bewaffnetem Boden = DENY); Truth-Anker `attest_payment_journal_tip` in `payments/journal_chain.py`, den `scripts/truth_anchor_run.py` statt `attest_ln_ops_tip` verankert (sonst waere keine Geldbewegung mehr on-chain); Empfangspfad nach `app/lightning/receive_gate.py` EXTRAHIERT statt migriert, weil `PaymentService.create_invoice` den anonymen `/oracle`-Mint hinter den Geldjournal-Lock schoebe (BL-2) und die Memo-Semantik `kai-oracle:{scope}` die Einnahmenzuordnung traegt.
