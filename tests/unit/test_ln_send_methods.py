...
_SENDS = [
    ...
    lambda c: keysend(dest_pubkey_hex="02abababababababababababababababababababababababababababababababab", amt_sat=100, dry_run=False, confirm=False, cfg=c),
]
...
async def test_executed_error_is_audited_not_disabled(monkeypatch) -> None:
    ...
    with patch("app.lightning.value_layer._build_client"):
        r = await send_coins(
            addr="bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4", amount_sat=1, dry_run=False, confirm=True, cfg=_cfg(False)
        )
