...
class TestMessariAdapterFetch:
    ...
    async def test_fetch_success_filters_and_returns_documents(self) -> None:
        ...
        # Verify BTC document
        btc_doc = next(d for d in result.documents if d.crypto_assets == ["BTC/USDT"])
        ...
        assert btc_doc.tickers == []
        assert btc_doc.crypto_assets == ["BTC/USDT"]
        assert btc_doc.categories == ["Payment"]
        assert btc_doc.tags == ["pow", "store_of_value"]
        assert "hasNews=True" in btc_doc.raw_text

        # Verify SOL document
        sol_doc = next(d for d in result.documents if d.crypto_assets == ["SOL/USDT"])
