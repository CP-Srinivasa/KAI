...
def test_der_pruefer_findet_ueberhaupt_verweise() -> None:
    ...
    assert len(verlangt) >= 8, f"nur {len(verlangt)} os.environ-Verweise: {sorted(verlangt)}"
...
def test_deepseek_bleibt_eine_zusaetzliche_transport_probe() -> None:
    ...
    routen = (REPO / "app" / "ai" / "routes.py").read_text(encoding="utf-8")
    ...
    for quelle in (control_plane, routen):
        assert "kai-deepseek" not in quelle
        assert "KAI_LITELLM_DEEPSEEK_MODEL" not in quelle


def test_deepseek_vertrag_enthaelt_keinen_schluessel() -> None:
    """Nur Name und Modelle sind dokumentiert; der Schluessel bleibt leer."""
    vorlage = VORLAGE.read_text(encoding="utf-8")
    ...
    schluessel = [z for z in vorlage.splitlines() if z.startswith("DEEPSEEK_API_KEY=")]
    assert schluessel == ["DEEPSEEK_API_KEY="], schluessel
