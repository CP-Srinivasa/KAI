...
async def test_feeder_filters_and_feeds_candidates(
    ...
    assert seen_runs[0]["analysis_result"].affected_assets == ["AVAX/USDT"]
...
@pytest.mark.asyncio
async def test_feeder_does_not_mark_non_filled_cycle_as_fed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    settings = AppSettings()
    settings.technical_paper = TechnicalPaperSettings(enabled=True, min_strength=0.7)
    monkeypatch.setattr(feeder, "get_settings", lambda: settings)
    ...
    async def fake_run_once(*, symbol, mode, analysis_result, analysis_source):
        return LoopCycle(
            cycle_id="cyc_not_filled",
            started_at=now_utc.isoformat(),
            completed_at=now_utc.isoformat(),
            symbol=symbol,
            status=CycleStatus.PAPER_CAP_REACHED,
    ...
    assert feeder.load_fed_ids(fed_file) == set()
