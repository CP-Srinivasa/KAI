...
def secondary_benchmark_family() -> list[tuple[str, Decider]]:
    """The twelve existing rules PLUS the new one, under next-open execution.

    ``SECONDARY_BENCHMARK - NON_GATING - NON_PROMOTIONAL``.

    It answers a separate and worthwhile question: is the new signal actually
    special, or does the more realistic next-open execution move the entire TA
    family? With twelve rules that were uniformly negative under close-to-close,
    that is diagnostically first-rate.

    What it must never do, in either direction: it may not rescue a failed
    primary test (a survivor here is a NEW hypothesis needing a NEW registration
    and a NEW future window), and it may not retroactively re-label a passed one
    (the other twelve were never part of its research question).
    """
    return [*default_hypotheses(), *primary_confirmatory_hypothesis()]
