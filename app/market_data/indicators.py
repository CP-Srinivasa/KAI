"""Market data indicators (ATR, SMA, etc.)."""
...
def compute_sma(candles: list[OHLCV], period: int = 200) -> float | None:
    """
    Compute Simple Moving Average (SMA) for a given list of OHLCV candles.
    Returns None if there are not enough candles.
    """
    if not candles or len(candles) < period:
        return None
    ...
    sorted_candles = sorted(candles, key=lambda c: c.timestamp_utc)
    closing_prices = [c.close for c in sorted_candles[-period:]]
