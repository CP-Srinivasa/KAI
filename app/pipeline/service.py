...
async def run_okx_announcements_pipeline(
    *,
    session_factory: async_sessionmaker[AsyncSession],
    keyword_engine: KeywordEngine,
    provider: BaseAnalysisProvider | None = None,
    shadow_provider: BaseAnalysisProvider | None = None,
    source_id: str = "okx_announcements",
    source_name: str = "OKX Announcements",
    url: str | None = None,
    dry_run: bool = False,
) -> PipelineRunStats:
    """Fetch OKX exchange announcements, persist, analyze, and alert.

    Drop-in equivalent of run_newsdata_pipeline for the OKX announcements API
    (source-scout 2026-06-14: exchange listings = highest-impact directional
    class). Non-directional notices are filtered downstream by eligibility.
    """
    ...
    from app.integrations.okx_announcements.adapter import (
        ...
        OKXAnnouncementsAdapter,
    )

    okx_url = url or OKX_ANNOUNCEMENTS_URL
    adapter = OKXAnnouncementsAdapter(
        SourceMetadata(
            source_id=source_id,
            source_name=source_name,
            source_type=SourceType.NEWS_API,
    ...
    # Stage 1: Fetch via OKX announcements API
    fetch_result = await adapter.fetch()
    if not fetch_result.success:
        logger.warning("okx_announcements_pipeline_fetch_failed", error=fetch_result.error)
        return PipelineRunStats(
            source_id=source_id,
            url=okx_url,
            fetched_count=0,
    ...
    logger.info(
        "okx_announcements_pipeline_fetched",
        announcements=len(fetch_result.documents),
    )
        ...
        async with session_factory.begin() as session:
            ...
            for res in pipeline_results:
                ...
                try:
                    ...
                    logger.warning(
                        "okx_announcements_pipeline_update_failed",
                        doc_id=str(res.document.id),
                        error=str(exc),
                    )
                    failed_count += 1
