...
| 4 | POLICY / DECISION | `app/execution/entry_policy.py` (`EntryRoute`) → `app/risk/engine.py` (`RiskEngine`) → `app/risk/promotion_gate.py` / `churn_killer.py` | Die Kette existiert nur als Aufrufreihenfolge in `app/orchestrator/trading_loop.py` (2.276 LOC) — **FIX (dokumentiert, kein Refactor)**. `app/security/governance/gates.py` ist **kein** Kettenglied: `trading_loop.py` importiert `app.security.governance` nicht (verifiziert 2026-09-04) — geprüftes Primitiv ohne Produktionsaufrufer. `app/governance/third_party_gate.py` = **absichtlich inert** (ADR 0013 / ADR 0014 G-3P; Ratchet `tests/unit/test_frontier_boundaries.py::test_third_party_settings_never_consumed_without_guard`) — fan-in 0 ist die Erfüllung der Regel, nicht ihre Verletzung. `app/orchestrator/governed_decision.py` = **entfernt 2026-09-04** (fan-in 0, Sidecar-Leser unerreichbar) |
...
| 7 | AUDIT / TRUTH CHAIN | `app/audit/kai_audit_service.py` (validiert, append-only, Lock) + `app/truth/` Attestation-Ledger (`artifacts/truth/attestation_ledger.jsonl`, Hash-Chain, live seq 118) | `decision_journal_chain.jsonl` + `app/audit/decision_chain.py` **entfernt 2026-09-04** — die Kette hatte keinen Produktionsschreiber, das Panel `/dashboard/api/audit-chain` konnte strukturell nur `empty` liefern; das KPI rechnet jetzt den Attestation-Ledger nach. `decision_journal.jsonl` bleibt (Approval-Mode-Pfad: 2 Operator-Endpoints + MCP + CLI + `signal-trail`), auf dem Pi 16 Zeilen, letzter Eintrag 2026-04-28 → **QUARANTINE** (Env-Entscheidung `OPERATOR_SIGNAL_APPEND_DECISION_ENABLED`, kein Struktur-Defekt); `app/truth` + `integrity` + `compliance` gehören inhaltlich zu `audit` → **MERGE** (DEFER) |
...
| Audit Chain | `tests/unit/test_truth_ledger*.py`, `test_kai_audit_service.py`, `test_audit_chain_status.py` (Attestation-Ledger-KPI); **Lücke:** `paper_execution_audit.jsonl` und `kai_audit.jsonl` sind append-only + validiert, aber **nicht hash-verkettet** (DEFER, § 12) |
...
| **QUARANTINE** | `app/intelligence/`, `app/research/` (+ Prereg-Timer `forecaster-*`, `prereg-maturity`, `edge-ic-*`, `min-turnover-calibration`, `exploration-coverage`), `app/chain/` + Lightning-Revenue-Module (`l402`, `selfpay`, `earnings_*`, `demand_*`, `treasury`, `reputation`; + `ln-scb-monitor`, `oracle-earnings-booking`) — `app/lightning/{client,adapter,cache,golive_preflight,receive_gate,receive_ledger}` sind der Rail-Adapter bzw. der Empfangspfad und damit KEEP; `value_layer`/`control_gate`/`policy`/`idempotency_store`/`ops_annotations`/`plan_guards` sind am 2026-09-04 mit ADR 0018 §12 PR 1 GELÖSCHT, `ops_ledger`, `reconciliation` + `reconcile_dual` sind am 2026-09-07 mit PR 2 GELÖSCHT (`artifacts/ln_ops_ledger_v2.jsonl` bleibt als Archiv liegen), `app/premium/` + Fastlane (+ `premium-healthcheck`, `premium-latency-audit`), Live-Exchange-Teilbaum, `decision_journal` (Kette entfernt 2026-09-04), `kai-tv-auto-promote.timer` (79 Tage grün bei 100 % Rejects — der Shadow-Feed ersetzt ihn), Branch `codex/llm-router-migration-20260901`, `test/regtest-ln/`, 12 nur-in-Docs-referenzierte Skripte |
...
**KAI SOVEREIGN VALUE-OS v0.1 (Payment Fabric, 2026-09-03, ADR 0018)** — gebaut als eigene Domäne `app/payments/` **auf** dem Kern, nicht im Kern: Payment Control Plane (PaymentIntent → Policy → Authorization → Rail-Execution → Settlement → Reconciliation → Proof), hash-verkettetes Geldjournal `artifacts/payments/payment_journal.jsonl`, Modi SIMULATION/SHADOW/LIVE, `/payments/*`, `/health/payment`; `app/lightning/` ist darunter der Lightning-Rail-Adapter (Client, Credentials, TLS-Boot) und importiert `app/payments` nie. v0.1-Aktionen: `pay_invoice` (BOLT11) + Invoice; keysend/on-chain/Channel bleiben DEFERRED und per Policy geblockt. Fiat/SEPA/PSP: nur Architektur (`docs/PAYMENT_FIAT_BRIDGE_ARCHITECTURE.md`, DESIGNED), kein Modul. Bestands-Rückbau (~2,7k LOC Alt-Geldpfad) folgt nach 7 Tagen Dual-Read (ADR 0018 §12).
...
**PR / CI / Deploy:** PR [#850](https://github.com/CP-Srinivasa/KAI/pull/850) (205 Dateien, +4.008/−19.047) · CI-Lauf `33675092177`: 9/9 Checks grün (Lint 40 s · Tests 9 min 7 s · Benchmarks 3 min 25 s · Security 41 s · Hygiene/Secret-Guard 12 s · Type-Check 1 min 8 s · Frontend 57 s · PR-Completeness · PR-Claims) · Squash-Merge auf Mainline **`61b1b4d2`** am 2026-09-02 19:54 UTC.
Deploy `kai_deploy.sh --restart kai-server` (2026-09-02 19:55 UTC): Urteil **DEPLOY_HOLD** (`SYSTEMD_CHANGE_REQUIRES_OPERATOR:35` — die 35 `EnvironmentFile=`-Härtungen warten auf `sudo bash scripts/pi_apply_systemd_units.sh`), Code auf dem Pi = `61b1b4d2`, `kai-server` per Broker neu gestartet. Nachbedingungen direkt geprüft: `/health` → `{"status":"ok","runtime_commit":"61b1b4d2…","checkout_commit":"61b1b4d2…","drift_commits":0}` · `kai-server`, `kai-agent-worker`, `kai-tg-listener`, `cloudflared`, `kai-entry-watch`, `kai-liquidation-stream` alle `active` · `GET /health/ai` → 200, Kette `primary=[openai, gemini]`, `shadow=[anthropic]`, Zustand openai `ok` (141 Calls im Fenster), anthropic `ok` (105), gemini `unknown` (0 — noch nie gebraucht) · `GET /health/config` → 200, `app.env=development`, `execution.entry_mode=paper`, `db.url=sqlite+aiosqlite:///…`, `openai_api_key=(set:…)`, `APP_ENV` **nicht** in `explicit` (bestätigt den Operator-Schritt aus § 8).
Bekannte Unschärfe (UNVERIFIED Ursache): `/health/ai` listet neben den LLM-Providern zwei Einträge `CNBC`/`Defillama` (je 1 Call) — ein Schreiber trägt Quellnamen ins `provider`-Feld des Telemetriestroms; der Snapshot sollte auf konfigurierte Provider oder `purpose` filtern (Folge-Fix, klein).
