...
| **QUARANTINE** | `app/intelligence/`, `app/research/` (+ Prereg-Timer `forecaster-*`, `prereg-maturity`, `edge-ic-*`, `min-turnover-calibration`, `exploration-coverage`), `app/chain/` + Lightning-Revenue-Module (`l402`, `selfpay`, `earnings_*`, `demand_*`, `treasury`, `reputation`; + `ln-scb-monitor`, `oracle-earnings-booking`) — `app/lightning/{client,adapter,cache,plan_guards,golive_preflight}` sind seit 2026-09-03 der Rail-Adapter unter `app/payments/` (ADR 0018) und damit KEEP; `value_layer`/`ops_ledger`/`reconciliation`/`control_gate`/`policy`/`idempotency_store` laufen im Dual-Read und sind DELETE-Kandidaten nach 7 Tagen, `app/premium/` + Fastlane (+ `premium-healthcheck`, `premium-latency-audit`), Live-Exchange-Teilbaum, `decision_journal` (Kette entfernt 2026-09-04), `kai-tv-auto-promote.timer` (79 Tage grün bei 100 % Rejects — der Shadow-Feed ersetzt ihn), Branch `codex/llm-router-migration-20260901`, `test/regtest-ln/`, 12 nur-in-Docs-referenzierte Skripte |
