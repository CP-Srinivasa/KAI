...
- `app/lightning/client.py::list_payments` → read-only LND-`GET /v1/payments` mit explizitem `include_incomplete` und validierter Pagination als `LndPaymentPage`; noch ohne Konsument (T6a-Voraussetzung für den separaten Reconciliation-Timer)
