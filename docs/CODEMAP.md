...
- `app/observability/operator_board_live.py` → `build_live_board`/`open_preregs`/`curated_is_stale` (rein, kein I/O) — LIVE-Hälfte von `/dashboard/api/operator-board`: `prereg_ledger.jsonl` minus `prereg_verdicts.jsonl` + Reife aus `research/prereg_maturity.py`. ⚠ `INSUFFICIENT_N` ist NICHT terminal (Claim bleibt offen); Stale-Alarm nur bei OFFENEN kuratierten Punkten, Chronik (`status: done`) veraltet nie
