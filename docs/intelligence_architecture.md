...
| I-291 | `PaperPortfolio` (mutable runtime state) MUST NEVER be directly exposed to any operator surface, MCP tool, CLI command, or Telegram handler. Only `PaperPortfolioSnapshot` (frozen, read-only projection) may cross the boundary into operator-facing surfaces. This is the paper portfolio equivalent of the MarketDataSnapshot pattern. |
| I-292 | `PaperPortfolioSnapshot`, `PositionSnapshot`, and `ExposureSummary` MUST all be frozen dataclasses with `execution_enabled=False` and `write_back_allowed=False` as non-overridable fields. Any portfolio surface response that omits or sets these fields to True MUST be treated as a configuration error and rejected fail-closed. |
| I-293 | The canonical source of truth for portfolio state reconstruction is `artifacts/paper_execution_audit.jsonl`. `build_paper_portfolio_snapshot_from_audit()` MUST replay `order_filled` events from this JSONL to reconstruct current positions. No live `PaperExecutionEngine` instance may be accessed via MCP or CLI. |
| I-294 | Mark-to-Market (MtM) enrichment is optional and fail-open for the snapshot itself but fail-closed per position. If `MarketDataSnapshot.is_stale=True` or `available=False`, the corresponding `PositionSnapshot.is_mark_to_market` MUST be `False` and `current_price` MUST be `None`. The portfolio snapshot MUST still be returned — MtM failure is NOT a fatal error. |
| I-295 | `ExposureSummary` is a derived projection of `PaperPortfolioSnapshot`. It MUST NOT have an independent data source, independent JSONL path, or independent market data fetch. The only path to an `ExposureSummary` is: audit_jsonl → `PaperPortfolioSnapshot` → `ExposureSummary`. No shortcut is permitted. |
| I-296 | Telegram `/positions` MUST be backed by `get_paper_portfolio_snapshot` MCP after Sprint 40. The provisional `get_handoff_collector_summary` backing is superseded. `TELEGRAM_CANONICAL_RESEARCH_REFS["positions"]` MUST reference `"research paper-portfolio-snapshot"`. |
| I-297 | Telegram `/exposure` MUST be backed by `get_portfolio_exposure_summary` MCP after Sprint 40. The stub implementation is no longer acceptable. `"exposure"` MUST appear in `_READ_ONLY_COMMANDS` and `TELEGRAM_CANONICAL_RESEARCH_REFS["exposure"]` MUST reference `"research portfolio-exposure"`. |
| I-298 | `get_paper_portfolio_snapshot` and `get_portfolio_exposure_summary` MUST be registered in `_CANONICAL_MCP_READ_TOOL_NAMES`. They MUST NOT be in `_GUARDED_MCP_WRITE_TOOL_NAMES`. No portfolio surface operation is a write operation. |
| I-299 | `position_id` in `PositionSnapshot` MUST be computed deterministically as `"pos_" + sha1(symbol + opened_at)[:12]`. The same position (same symbol, same opened_at) MUST always produce the same `position_id` regardless of when the snapshot is built. This ensures stable references across repeated reads. |
| I-300 | The paper portfolio surface layer (`app/research/portfolio_surface.py`) MUST NOT import from `app/execution/paper_engine.py`. It may import `PaperPosition` and `PaperPortfolio` from `app/execution/models.py` for type annotations in builder functions, but it MUST NOT call any method that triggers order execution, position mutation, or state write. The boundary is: models OK, engine forbidden. |
