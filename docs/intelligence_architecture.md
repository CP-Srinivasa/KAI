...
| current_sprint | `PH5B_LOW_SIGNAL_CLUSTER_ANALYSIS (ready to close)` |
| next_required_step | `PH5B_RESULTS_REVIEW_AND_PH5C_DEFINITION` |
| baseline | `1609 passed, ruff clean` |
| phase4_status | `CLOSED (D-87, 2026-03-24)` |
| v4_dual_write_status | `closed (N-4 closed)` |
| architecture_status | three-tier stack unchanged; PH5B diagnostics complete; PH5C blocked until PH5B review closes |

---

## PH5B Ready To Close (2026-03-24)
...
- PH5B artifacts are available for formal closeout review.
- Recommended next sprint: `PH5C_FILTER_BEFORE_LLM_BASELINE` as a narrow pre-LLM filter.

---

## PH4A/PH4B (closed ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â frozen anchors)
...
- PH4B review confirmed root cause: Tier-1 assigns default scores on keyword miss ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â structural keyword coverage blindness.
- Divergence profile: `18` severe (|delta|>=5), `40` moderate, `11` minor; `0.00%` tag overlap.
- PH4A (ÃƒÂ‚Ã‚Â§67) and PH4B (ÃƒÂ‚Ã‚Â§68, ÃƒÂ‚Ã‚Â§69) are immutable comparison anchors. No re-execution permitted.

## PH4C (closed ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ÃƒÂ‚Ã‚Â§70 frozen audit anchor, D-66)

- Sprint: `PH4C_RULE_KEYWORD_COVERAGE_AUDIT` (diagnostic-only). Formally closed D-66.
- Contract: `docs/contracts.md ÃƒÂ‚Ã‚Â§70` (frozen immutable anchor).
...
## PH4D (closed ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ÃƒÂ‚Ã‚Â§71 frozen anchor, D-68)

- Sprint: `PH4D_TARGETED_KEYWORD_EXPANSION_BASELINE` (targeted keyword expansion). Formally closed D-68.
- Contract: `docs/contracts.md ÃƒÂ‚Ã‚Â§71` (frozen immutable anchor).
- Execution outcomes (frozen):
  - scope applied: macro/finance (+24), regulatory/legal (+18), AI/technology (+14)
  - re-measurement (69 paired docs): zero `29ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™26`, low `27ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™25`, good `13ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™18`, regressions `0`
  - index growth: `507ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™555` (+48)
- Zero-hit review (frozen): 26 remaining; 5 true rule gaps ÃƒÂ‚Ã‚Â· 21 correctly low-value noise.

## Phase 4 Interim Review (closed ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ÃƒÂ‚Ã‚Â§72, D-65/D-66)
...
- Contract: `docs/contracts.md ÃƒÂ‚Ã‚Â§72` (frozen immutable anchor).

## PH4E Closed Sprint (ÃƒÂ‚Ã‚Â§73 frozen anchor ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â D-67)

- Sprint: `PH4E_SCORING_CALIBRATION_AUDIT` (diagnostic scoring audit). **Formally closed D-67.**
- Contract: `docs/contracts.md ÃƒÂ‚Ã‚Â§73` (immutable frozen anchor ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â no re-execution permitted).
...
- Root cause: **defaults by design** ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â RuleAnalyzer (`app/analysis/rules/rule_analyzer.py` lines 13-18) explicitly documents these as LLM-dependent fields.
...
## PH4F Closed Sprint (frozen anchor ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ÃƒÂ‚Ã‚Â§74, D-69)

- Sprint: `PH4F_RULE_INPUT_COMPLETENESS_AUDIT`. **Formally closed.**
- Contract: `docs/contracts.md ÃƒÂ‚Ã‚Â§74` (closed immutable anchor).
- Execution findings (locked):
  - Production Tier-1 path = `_build_fallback_analysis()` in `pipeline.py` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â NOT `RuleAnalyzer.analyze()`
  ...
  - `tags`: empty 69/69 paired docs (no keyword hits ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ no tag output)
  - `relevance_score`: at default floor 56/69 docs (81.2%)
- LLM-layer coverage verdict: no triggering gap; gap is `provider=None` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ fallback ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ hard defaults.
...
- Contract: `docs/contracts.md Ã‚Â§75` (closed; frozen anchor).
- Execution findings (locked):
  - Relevance-floor fallback intervention: **retained** (applied successfully)
  - Actionable heuristic intervention: **reverted** ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â violates I-13 invariant (rule-only priority ceiling max 5)
  - I-13 invariant: `test_rule_only_priority_ceiling_is_at_most_five` enforces priority ÃƒÂ¢Ã¢Â€Â°Ã‚Â¤ 5 for rule-only analysis
...
- Contract: `docs/contracts.md Ã‚Â§76` (closed; frozen anchor D-74/75).
...
## PH4I Closed Sprint (Ã‚Â§77 frozen anchor)

- Sprint: `PH4I_FALLBACK_MARKET_SCOPE_ENRICHMENT`. **Formally closed (D-78).**
- Contract: `docs/contracts.md Ã‚Â§77` (closed frozen anchor).
- Outcome: `market_scope` enrichment in fallback path completed.

## PH4J Closed Sprint (Ã‚Â§78 frozen anchor Ã¢Â€Â” D-81)

- Sprint: `PH4J_FALLBACK_TAGS_ENRICHMENT`. **Formally closed (D-81).**
- Contract: `docs/contracts.md Ã‚Â§78` (closed; frozen anchor).
- Execution outcomes (locked):
  - keyword-hit tags: `4 Ã¢Â†Â’ 7`
  - zero-hit tags: `1 Ã¢Â†Â’ 4`
  - assets-only tags: `0 Ã¢Â†Â’ 4`
...
## PH4K Closed Sprint (Ã‚Â§79 frozen anchor Ã¢Â€Â” D-84)

- Sprint: `PH4K_TAG_SIGNAL_UTILITY_REVIEW`. **Formally closed (D-84).**
- Contract: `docs/contracts.md Ã‚Â§79` (closed; frozen anchor).
...
- Contract: `docs/contracts.md Ã‚Â§81`.
- Scope: Complete V-4 Ã¢Â€Â” session-factory refactor, PortfolioStateRecord write on fill, DB-primary read.
- Key invariants:
...
Tier depth scales with available resources ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â the system degrades gracefully, never silently.
...
```
ÃƒÂ¢Ã¢Â€ÂÃ…Â’ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ‚Â
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Tier 3 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â External LLM Provider (current default)              ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  OpenAI / Anthropic / Gemini                                    ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Full output: all scores, narrative, actionable classification  ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Priority range: 1ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Âœ10  ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Produces SignalCandidates             ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Cost: API call per document                                    ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€ÂÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ‹Âœ
                               ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš fallback if unavailable
ÃƒÂ¢Ã¢Â€ÂÃ…Â’ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€Â“Ã‚Â¼ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ‚Â
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Tier 2 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Internal Companion Model (Sprint 5 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â planned)        ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Local inference: GGUF / ONNX / vLLM endpoint                  ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Subset output: sentiment, relevance, impact (conservative)     ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Priority range: 1ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Âœ8   ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Can produce SignalCandidates          ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Cost: local compute, no API key required                       ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€ÂÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ‹Âœ
                               ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš fallback if unavailable
ÃƒÂ¢Ã¢Â€ÂÃ…Â’ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€Â“Ã‚Â¼ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ‚Â
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Tier 1 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â RuleAnalyzer (implemented, Sprint 4C)                ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Deterministic: keyword matching + heuristics                   ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Conservative output: relevance only, all others at floor       ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Priority range: 1ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Âœ5   ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Never produces SignalCandidates       ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  Cost: zero (no model, no network)                              ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
ÃƒÂ¢Ã¢Â€ÂÃ¢Â€ÂÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ¢Â‚Â¬ÃƒÂ¢Ã¢Â€ÂÃ‹Âœ
```

All three tiers converge on the same downstream contract:
`CanonicalDocument ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ AnalysisResult ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ apply_to_document() ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ research outputs / alert gate`
...
## Tier 1 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â RuleAnalyzer
...
| Field | Value |
|-------|-------|
| `relevance_score` | keyword-density heuristic (0.0ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Âœ0.6) |
...
**Priority ceiling proof** ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â with all conservative defaults:
```
raw = (relevanceÃƒÂƒÃ¢Â€Â”0.30) + (impactÃƒÂƒÃ¢Â€Â”0.30) + (noveltyÃƒÂƒÃ¢Â€Â”0.20) + (actionableÃƒÂƒÃ¢Â€Â”0.15) + (qualityÃƒÂƒÃ¢Â€Â”0.05)
    ÃƒÂ¢Ã¢Â€Â°Ã‚Â¤ (0.60ÃƒÂƒÃ¢Â€Â”0.30) + (0.0ÃƒÂƒÃ¢Â€Â”0.30) + (0.5ÃƒÂƒÃ¢Â€Â”0.20) + (0ÃƒÂƒÃ¢Â€Â”0.15) + (1.0ÃƒÂƒÃ¢Â€Â”0.05)
    = 0.18 + 0.0 + 0.10 + 0 + 0.05 = 0.33 ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ priority = round(0.33ÃƒÂƒÃ¢Â€Â”9)+1 = 4
```
Maximum achievable (max relevance + max quality): ~5. SignalCandidate threshold is 8 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â gap is intentional.
...
Fix: Sprint 4C Task 4.10 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â relax the guard.

---

## Tier 2 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Internal Providers

Tier 2 has two distinct implementations. Both implement `BaseAnalysisProvider` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â zero pipeline changes required.

### Tier 2a ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â InternalModelProvider (`APP_LLM_PROVIDER=internal`)

**Status**: ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ Implemented (`app/analysis/internal_model/provider.py`)

```
provider_name = "internal"
analysis_source ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ INTERNAL
```
...
### Tier 2b ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â InternalCompanionProvider (`APP_LLM_PROVIDER=companion`)

**Status**: ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ Implemented (`app/analysis/providers/companion.py`)
...
HTTP client to a local OpenAI-compatible endpoint (e.g. Ollama, llama.cpp, vLLM ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â localhost only).
...
**Priority range with companion**: Typical strong output ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ priority 8 (SignalCandidate threshold).

### Settings (Implemented)

```python
# app/core/settings.py ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ProviderSettings
...
"internal"   ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ InternalModelProvider(keyword_engine)       # always returns instance
"companion"  ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ InternalCompanionProvider(endpoint, model)  # returns None if endpoint not set
```

### Security Constraints

- `companion_model_endpoint` MUST be `localhost` or an explicitly allowlisted internal address.
- No external network calls from companion provider ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â local inference only.
- No API keys for companion model ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â authentication is endpoint-level (internal network).
- Validation at settings load time: reject external URLs for companion endpoint.

---

## Tier 3 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â External LLM Provider
...
**Priority range**: 1ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Âœ10. All scores available. Full signal eligibility.
...
```
APP_LLM_PROVIDER env var ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ create_provider() ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ provider | None
if None ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ AnalysisPipeline runs without LLM ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ RuleAnalyzer fallback result
...
# provider_name ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ "ensemble(openai,internal)" (compound, for traceability)
# model ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ actual winner's provider_name (tracked at runtime)
```

### Sprint 5C ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â EnsembleProvider Winner-Traceability ÃƒÂ¢Ã…Â“Ã¢Â€Â¦

Post-`analyze()` resolution via duck-typing:
- `_resolve_runtime_provider_name(provider)` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â reads `active_provider_name` from `EnsembleProvider` after `analyze()` completes
- `_resolve_trace_metadata(provider)` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â reads `provider_chain` from `EnsembleProvider` to build `ensemble_chain`
- `_resolve_analysis_source(provider_name)` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â string-based, maps winner name to `AnalysisSource`
...
### Enum ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ Implemented

```python
# app/core/enums.py
class AnalysisSource(StrEnum):
    RULE = "rule"                  # Tier 1 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â fallback / rule-based heuristics
    INTERNAL = "internal"          # Tier 2 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â InternalModelProvider or InternalCompanionProvider
    EXTERNAL_LLM = "external_llm"  # Tier 3 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â OpenAI / Anthropic / Gemini
```

### Optional Field + Backward-Compat Property ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ Implemented (Sprint 5B)

```python
# app/core/domain/document.py
doc.analysis_source: AnalysisSource | None  # set by apply_to_document() via pipeline
doc.effective_analysis_source: AnalysisSource  # @property ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â backward-compat accessor
```

`effective_analysis_source` derivation (fallback for legacy rows without DB column):
- `doc.analysis_source is not None` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ return it directly
- `doc.provider in {None, "fallback", "rule"}` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `RULE`
- `doc.provider in {"internal", "companion"}` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `INTERNAL`
- `doc.provider.startswith("ensemble(")` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `INTERNAL` (pre-5C composite guard)
- else ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `EXTERNAL_LLM`

### DB Column ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ Implemented (Sprint 5B, migration 0006)

`canonical_documents.analysis_source VARCHAR(20)` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Alembic migration `0006_add_analysis_source_column.py`.
...
Teacher-only dataset
      ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  analysis_source = EXTERNAL_LLM
      ÃƒÂ¢Ã¢Â€Â“Ã‚Â¼
Distillation-ready corpus
      ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš
      ÃƒÂ¢Ã¢Â€Â“Ã‚Â¼
Internal benchmark export
      ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  analysis_source = INTERNAL
      ÃƒÂ¢Ã¢Â€Â“Ã‚Â¼
Rule baseline export
      ÃƒÂ¢Ã¢Â€ÂÃ¢Â€Âš  analysis_source = RULE
      ÃƒÂ¢Ã¢Â€Â“Ã‚Â¼
...
|--------|:---:|:---:|:---:|
| `ResearchBrief.key_documents` | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| `ResearchBrief.top_actionable_signals` | ÃƒÂ¢Ã‚ÂÃ…Â’ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ (if priority ÃƒÂ¢Ã¢Â€Â°Ã‚Â¥ 8) | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| `SignalCandidate` via `extract_signal_candidates()` | ÃƒÂ¢Ã‚ÂÃ…Â’ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| `direction_hint != "neutral"` | ÃƒÂ¢Ã‚ÂÃ…Â’ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| `impact_score > 0` | ÃƒÂ¢Ã‚ÂÃ…Â’ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| Full narrative / explanation | ÃƒÂ¢Ã‚ÂÃ…Â’ | Partial | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
...
|--------|-----------|--------|
| 4C | Relax `apply_to_document()` guard ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Tier 1 results persisted | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 4C | `analyze_pending` None-guard ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â FAILED instead of silent | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 4D | `InternalModelProvider` (heuristic, zero deps) | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 4D | `EnsembleProvider` (ordered fallback, first success wins) | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 4D | `InternalCompanionProvider` (HTTP to local model endpoint) | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 4D | Factory routing: `"internal"` / `"companion"` / ensemble | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 5A | `InternalCompanionProvider` settings fields + localhost validation | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 5B | `AnalysisSource` enum + `doc.analysis_source` field | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 5B | `analysis_source` DB migration (migration 0006) + ORM column | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 5B | `effective_analysis_source` property ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â backward-compat accessor | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 5B | Pipeline: `_resolve_analysis_source()` + `apply_to_document()` write | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 5B | Provenance in research outputs: briefs, signals, datasets | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 5C | `EnsembleProvider` Winner-Traceability ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â post-analyze resolution | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 5C | `doc.provider` = winner name; `doc.metadata["ensemble_chain"]` = full list | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 6 | Dataset construction + evaluation harness + distillation readiness | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 7 | Companion benchmark harness + promotion gate + artifact contract | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 8 | Controlled companion inference + tuning artifact flow + manual promotion | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 9 | Promotion audit hardening: I-34 automated (G6), gates_summary in record, artifact linkage | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
---
...
| 8 | Controlled companion inference + tuning artifact flow + manual promotion | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 9 | Promotion audit hardening: I-34 automated (G6), gates_summary in record, artifact linkage | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 10 | Companion shadow run: audit-only parallel inference, divergence JSONL, no routing influence | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 11 | Distillation harness: teacher/candidate/shadow combined readiness report, evaluation engine, distillation manifest | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 12 | Training Job Record: pre-training manifest, post-training eval link, promotion continuity, shadow schema canonicalization | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 13 | Evaluation Comparison + Regression Guard: pre/post model comparison, regression visibility (has_regression), comparison audit artifact, PromotionRecord.comparison_report_path, record-promotion --comparison (I-72), upgrade-cycle-status CLI | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 14 | Controlled A/B/C inference profiles + signal distribution contract (no auto-routing) | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 14C | Runtime Route Activation: ActiveRouteState, route-activate/route-deactivate CLI, I-90ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-93 | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 17 | analyze-pending Route Integration: route_runner.py, ActiveRouteState consumed by analyze-pending, ABCInferenceEnvelope per document ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ audit JSONL only (I-92, I-93) | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 18 | Controlled MCP Server: app/agents/mcp_server.py, 8 read tools + 3 guarded write tools, _resolve_workspace_path() workspace guard, I-94ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-100 | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 16 | Immutable Signal Handoff Layer: execution_handoff.py (SignalHandoff frozen dataclass), CLI: signal-handoff, JSONL batch export, I-105ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-108 | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 19 | Route-Aware Distribution: classify_delivery_class(), RouteAwareDistributionSummary, DistributionClassificationReport, DeliveryClassification, I-109ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-115 | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 20 | Consumer Collector & Acknowledgement Orchestration: execution_handoff.py (HandoffAcknowledgement, create/append/load_handoff_acknowledgement), distribution.py (HandoffCollectorSummaryReport, build_handoff_collector_summary), acknowledge_signal_handoff MCP (audit-only write, PermissionError on hidden), get_handoff_collector_summary MCP (read), CLI: handoff-acknowledge + handoff-collector-summary, I-116ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-122 | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 21 | Operational Readiness Surface: operational_readiness.py (OperationalReadinessReport, ReadinessIssue, RouteReadinessSummary, AlertDispatchSummary, ProviderHealthSummary, DistributionDriftSummary, OperationalArtifactRefs, build/save_operational_readiness_report), MCP: get_operational_readiness_summary (read-only), CLI: research readiness-summary, I-123ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-130 | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 22 | Provider Health & Distribution Drift Monitoring: operational_readiness.py bleibt der einzige Monitoring-Stack; MCP: get_provider_health(handoff_path, state_path, abc_output_path) + get_distribution_drift(handoff_path, state_path, abc_output_path) als read-only Readiness-Views (I-95, I-134), CLI: research provider-health + research drift-summary als Readiness-Views, operational_alerts.py superseded, I-131ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-138, contracts.md ÃƒÂ‚Ã‚Â§34 | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 23 | Protective Gates & Remediation Recommendations: operational_readiness.py (interne ProtectiveGateSummary/ProtectiveGateItem in OperationalReadinessReport) als einziger kanonischer Gate-Pfad, read-only advisory system, kein Execution-Hook, MCP: get_protective_gate_summary(...) + get_remediation_recommendations(...), CLI: research gate-summary + research remediation-recommendations, protective_gates.py superseded, I-139ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-145, contracts.md ÃƒÂ‚Ã‚Â§35 ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 24 | Artifact Lifecycle Management: artifact_lifecycle.py (ArtifactEntry, ArtifactInventoryReport frozen execution_enabled=False, ArtifactRotationSummary), build_artifact_inventory(artifacts_dir, stale_after_days=30), rotate_stale_artifacts(dry_run=True default, archive-only never-delete, policy-aware: protected skipped), MCP: get_artifact_inventory (read-only, workspace-confined), CLI: research artifact-inventory + research artifact-rotate (--dry-run default), I-146ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-152, contracts.md ÃƒÂ‚Ã‚Â§36 ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 25 | Safe Artifact Retention & Cleanup Policy: artifact_lifecycle.py erweitert ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ArtifactRetentionEntry (frozen, delete_eligible=False immer), ArtifactRetentionReport (frozen, execution_enabled=False, write_back_allowed=False, delete_eligible_count=0), ArtifactCleanupEligibilitySummary, ProtectedArtifactSummary. classify_artifact_retention() reine Klassifikation (I-160). Klassen: audit_trail/promotion/training_data/active_state/evaluation/operational/unknown ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ protected/rotatable/review_required. rotate_stale_artifacts() policy-aware (I-155). MCP: get_artifact_retention_report + get_cleanup_eligibility_summary + get_protected_artifact_summary. CLI: research artifact-retention. I-153ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-161, contracts.md ÃƒÂ‚Ã‚Â§37 ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| 26 | Artifact Governance/Review Surface: artifact_lifecycle.py bleibt der einzige Governance-/Review-Stack auf Basis des kanonischen Retention-Reports. Finale read-only Modelle/Slices: ArtifactRetentionReport, ArtifactCleanupEligibilitySummary, ProtectedArtifactSummary, ReviewRequiredArtifactSummary. MCP: get_artifact_retention_report + get_cleanup_eligibility_summary + get_protected_artifact_summary + get_review_required_summary. CLI: research artifact-retention + research cleanup-eligibility-summary + research protected-artifact-summary + research review-required-summary. Superseded: ArtifactGovernanceSummary, ArtifactPolicyRationaleSummary, get_governance_summary, get_policy_rationale_summary, research governance-summary. contracts.md ÃƒÂ‚Ã‚Â§38 ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |

---

## Invariants

> Full invariant list is canonical in `docs/contracts.md ÃƒÂ‚Ã‚Â§Immutable Invariants`.
> Intelligence-layer invariants (I-14 through I-33) are listed here for quick reference.

| ID | Rule |
|----|------|
| I-14 | `InternalCompanionProvider` implements `BaseAnalysisProvider` exactly ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â zero pipeline changes |
| I-15 | Companion model endpoint MUST be localhost or allowlisted ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â no external inference |
| I-16 | Distillation corpus uses only `analysis_source=EXTERNAL_LLM` documents as teacher signal |
| I-17 | Companion model `impact_score` cap: ÃƒÂ¢Ã¢Â€Â°Ã‚Â¤ 0.8 (conservative, not overconfident) |
| I-18 | `AnalysisSource` is set at result creation time ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â immutable after `apply_to_document()` |
| I-19 | Rule-only documents (`analysis_source=RULE`) NEVER serve as distillation teacher signal |
| I-20 | `InternalModelProvider.provider_name` is always `"internal"`, `recommended_priority` ÃƒÂ¢Ã¢Â€Â°Ã‚Â¤ 5, `actionable=False`, `sentiment_label=NEUTRAL` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â hard invariants, not configurable |
| I-21 | `InternalCompanionProvider.provider_name` is always `"companion"` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â distinct from `"internal"`. Factory routes `"internal"` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `InternalModelProvider`, `"companion"` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `InternalCompanionProvider` |
| I-22 | `EnsembleProvider` requires at least one provider. `InternalModelProvider` MUST be last for guaranteed fallback. All fail ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `RuntimeError` |
| I-23 | `EnsembleProvider.model` MUST return the winning provider's `provider_name` immediately after `analyze()` completes ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â this is the canonical winner signal |
...
| I-34 | `false_actionable_rate` is the 6th automated promotion gate (G6, ÃƒÂ¢Ã¢Â€Â°Ã‚Â¤ 0.05). Computed by `compare_datasets()`, enforced by `validate_promotion()` as `false_actionable_pass`. Supersedes original "manual, deferred" note. See I-46, `docs/contracts.md ÃƒÂ‚Ã‚Â§20`. |
| I-51 | Shadow run MUST NEVER call `apply_to_document()` or `repo.update_analysis()`. Zero DB writes to `canonical_documents`. |
| I-52 | Shadow run calls `InternalCompanionProvider.analyze()` directly ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â independent of `APP_LLM_PROVIDER`. Never a routing override. |
| I-53 | Shadow JSONL is a standalone audit artifact ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â not EvaluationReport input, not training corpus. |
| I-54 | Shadow run requires `companion_model_endpoint`. Absent ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ exit 0 (informational), not error. |
| I-55 | Divergence summary is informational only ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â never used for routing, gating, or output modification. |
...
| I-60 | `compute_shadow_coverage()` reads shadow records for divergence stats only ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â never calls `compare_datasets()`. |
| I-61 | `DistillationReadinessReport.shadow_coverage` is optional ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â absent shadow does not block distillation readiness. |
| I-62 | `build_distillation_report()` is pure computation ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â no DB reads, no LLM calls, no network. |
| I-63 | `TrainingJobRecord` is a pre-training manifest only ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â no training, no API calls, no weight updates. |
| I-64 | `TrainingJobRecord` status="pending" does not represent a trained model. Training is operator-external. |
| I-65 | Post-training evaluation MUST pass G1ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœG6 via `validate_promotion()`. No bypass. |
...
| I-70 | `EvaluationComparisonReport` is comparison artifact only ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â no routing, no promotion trigger, no gate bypass. |
| I-71 | `compare_evaluation_reports()` is pure computation ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â two JSON files only. No DB, no LLM, no network. |
| I-72 | Hard regression detected + `--comparison` passed to `record-promotion` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ RED warning printed. No auto-block. Operator decides. |
| I-73 | `compare-evaluations` exit 0 ÃƒÂ¢Ã¢Â€Â°Ã‚Â  promotable. `check-promotion` still required (I-36, I-65). |
| I-74 | Baseline and candidate must share same `dataset_type` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â mismatch ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `ValueError`. |
...
| I-76 | `UpgradeCycleReport.status` derived from artifact presence (`Path.exists()`) only ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â never auto-advanced by platform code. |
...
| I-78 | `UpgradeCycleReport.promotion_record_path` only set when operator explicitly supplies it ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â never auto-populated from env or settings. |
| I-79 | Each `UpgradeCycleReport` = one upgrade attempt. Separate files per cycle. No in-place overwrite (I-38 extends). |
| I-80 | Route profiles are declarative only ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â never self-activating. |
...
| I-151 | Stale detection uses file `mtime` only ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â no content inspection of artifact files. |
...
| I-211 | All 41 registered `@mcp.tool()` functions MUST be classified as canonical, active_alias, superseded, or workflow_helper. No tool may be unclassified. Classification is maintained in contracts.md ÃƒÂ‚Ã‚Â§44. (Updated Sprint 33.) |
...
| I-219 | MCP surface changes (adding or removing tools from read_tools or write_tools) MUST be reflected in contracts.md ÃƒÂ‚Ã‚Â§44 and intelligence_architecture.md within the same sprint. |
...
| I-235 | SignalÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™Order mapping MUST be deterministic: same signal + same prices ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ same order parameters. |
...
| I-261 | `app/core/schema_binding.py` is the schema integrity layer ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â it validates that the schema FILES themselves are structurally correct. It MUST NOT be confused with `runtime_validator.py` (payload validation). The two modules are complementary, not competing. |
...
| I-264 | `run_all_schema_validations()` in `schema_binding.py` MUST be callable at startup to detect schema drift. It returns a list of `SchemaValidationResult` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â one per check ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â and MUST NOT raise on failures (advisory, not fail-closed). |
...
| I-274 | Commands from non-admin `chat_id` values MUST be logged and fail-closed with "Unauthorized. This incident is logged." No command from an unauthorized chat_id may reach any handler. The response MUST be generic ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â no internal detail disclosed. |
...
| I-276 | The canonical Telegram command surface is defined in `TELEGRAM_INTERFACE.md`. Any sprint that changes the command surface (adds, removes, or reclassifies a command) MUST update both `TELEGRAM_INTERFACE.md` and `docs/contracts.md ÃƒÂ‚Ã‚Â§49` within the same sprint. |
| I-277 | Telegram commands are NOT MCP tools. They share no tool inventory with `app/agents/mcp_server.py`. A Telegram command that reads from a canonical MCP read surface calls the MCP function directly (via lazy import) ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â it MUST NOT route through the MCP tool dispatch layer. |
...
| I-283 | `MarketDataPoint.source` MUST be set by the adapter ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â never inferred, defaulted, or overwritten by the consumer. The source field is a provenance tag. It is NOT a routing signal and NOT a permission check. Signals derived from a MarketDataPoint SHOULD propagate the source value for traceability. |
...
| I-285 | All `BaseMarketDataAdapter` methods MUST implement the never-raise contract: transient fetch failures return `None` (get_ticker, get_price, get_market_data_point) or `[]` (get_ohlcv), never raise. Internal errors MUST be logged at WARNING level before returning the null value. health_check() returns `False` on any error ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â never raises. |
...
| I-288 | Market data adapter selection is explicit configuration (DI / settings). No automatic fallback chain between adapters is permitted. A `TradingLoop` or any consumer is constructed with exactly one adapter ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â it does not switch providers at runtime. Provider changes require explicit reconfiguration and restart. |
| I-289 | A real external market data adapter (e.g. Binance, Alpaca) MUST implement `BaseMarketDataAdapter` completely. Every abstract method MUST be overridden. Any unimplemented method MUST raise `NotImplementedError` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â not silently return `None`. Partial implementations are forbidden. |
| I-290 | All `MarketDataPoint.timestamp_utc` values MUST be UTC-aware datetimes. Naive datetimes are invalid. Adapters MUST ensure UTC-awareness before constructing the dataclass. Consumers that receive a naive timestamp MUST treat it as a data error and log a warning ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â they MUST NOT silently assume UTC. |

| I-291 | `PaperPortfolio` (mutable runtime state) MUST NEVER be directly exposed to any operator surface, MCP tool, CLI command, or Telegram handler. Only `PortfolioSnapshot` (frozen, read-only projection from `app/execution/portfolio_read.py`) may cross the boundary into operator-facing surfaces. `app/execution/portfolio_surface.py` is an internal TradingLoop helper ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â it MUST NOT be used as an operator surface. |
...
| I-299 | `PositionSummary.symbol` is the canonical position key in `PortfolioSnapshot`. Positions are keyed and sorted by symbol. There is no separate `position_id` field ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â the `symbol` field is stable and unique per open position in the paper portfolio (one position per symbol at a time). |
...
| I-303 | `run_trading_loop_once()` defaults to `provider="mock"`, which instantiates `MockMarketDataAdapter`. No real external adapter may be used in the default run-once path. The default `analysis_profile="conservative"` generates a non-actionable `AnalysisResult` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â this intentionally produces a `CycleStatus.NO_SIGNAL` cycle without paper order creation. |
...
| I-305 | `run_trading_loop_once()` creates a fresh `PaperExecutionEngine` per call (no portfolio replay from `paper_execution_audit.jsonl`). If a cycle completes with a simulated fill, the `PaperExecutionEngine` writes the fill to `execution_audit_path` (default: `artifacts/paper_execution_audit.jsonl`). This is correct behavior ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â run-once paper fills ARE part of the paper execution audit. |
...
| I-309 | `research trading-loop-run-once` CLI command MUST be registered (it is already declared in `FINAL_RESEARCH_COMMAND_NAMES`). Until registered, `test_research_command_inventory_matches_registration_and_help` fails ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â this is the Sprint 41 implementation blocker. Once registered, the test MUST pass. |
...
## Sprint 42+42C ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Telegram Webhook Hardening (I-311ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-320) ÃƒÂ¢Ã…Â“Ã¢Â€Â¦

> **Sprint 42C (2026-03-21)**: Invarianten korrigiert auf tatsÃƒÂƒÃ‚Â¤chliche Implementierung. I-311/I-312/I-313/I-315/I-316/I-318/I-319/I-320 wurden mit falschen Namen geschrieben und sind hier finalisiert.

| ID | Invariant |
|---|---|
| I-311 | Webhook-Hardening ist **vollstÃƒÂƒÃ‚Â¤ndig in `app/messaging/telegram_bot.py` integriert** (kein separates Modul). `TelegramWebhookProcessResult` (frozen dataclass: `accepted`, `processed`, `rejection_reason`, `update_id`, `update_type`) ist das kanonische Resultat-Modell. Kein separates Legacy-Validator-/Webhook-Modul. |
...
| I-313 | `webhook_secret_token=None/""` at construction ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `rejection_reason="webhook_secret_not_configured"` on every request ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â fail-closed. The `webhook_configured` property MUST be checked before processing. `webhook_signature_required: True` in runtime config is satisfied when `webhook_secret_token` is non-empty. |
| I-314 | Secret-Token mismatch ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `rejection_reason="invalid_secret_token"`. Missing header ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `rejection_reason="missing_secret_token_header"`. Secret comparison MUST use `hmac.compare_digest` (constant-time). Implemented as `_constant_time_secret_match()`. |
...
| I-316 | Replay deduplication uses an in-memory `OrderedDict` with FIFO eviction at `maxlen=2048` (not `deque`). A duplicate `update_id` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `rejection_reason="duplicate_update_id"`. Buffer is NOT persisted. Restart = empty buffer. This is safe: Telegram confirms delivery on HTTP 200. |
...
| I-318 | Rejected webhook requests MUST produce exactly one audit append to `artifacts/telegram_webhook_rejections.jsonl` (rejections only ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â not all requests). Accepted requests are covered by the existing `artifacts/operator_commands.jsonl`. No secrets or credentials in the audit. |
...
## Sprint 43+43C ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â FastAPI Operator API Surface (I-321ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-330) ÃƒÂ¢Ã…Â“Ã¢Â€Â¦

> **Sprint 43C (2026-03-21):** Invarianten basieren auf tatsÃƒÂƒÃ‚Â¤chlicher Implementierung in `app/api/routers/operator.py`. Sprint-43-Entwurf hatte andere Endpunkt-Namen und Webhook-Endpoints die nicht implementiert wurden.

| ID | Invariant |
|---|---|
| I-321 | `app/api/routers/operator.py` ist das kanonische Operator-API-Modul. Es enthÃƒÂƒÃ‚Â¤lt **keine eigene Business-Logik** ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ausschliesslich Delegation an bestehende MCP-Funktionen (`mcp_server.*`). Kein Datenbankzugriff, kein direkter Adapter-Zugriff, kein Scheduling. |
| I-322 | `require_operator_api_token` ist eine FastAPI-Dependency-Funktion (DI), gesetzt als `dependencies=[Depends(require_operator_api_token)]` auf dem gesamten Router. Sie ist NICHT ÃƒÂƒÃ‚Â¼ber die bestehende `app/security/auth.py`-Bearer-Middleware implementiert. Leer `APP_API_KEY` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ 503. Kein Header ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ 401. Falscher Token ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ 403. Tokenvergleich via `secrets.compare_digest` (constant-time). |
| I-323 | `GET /operator/status` und `GET /operator/readiness` sind **Aliases** ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â beide rufen `mcp_server.get_operational_readiness_summary()` ohne weitere Parameter auf. Kein inhaltlicher Unterschied. Beide sind read_only. |
| I-324 | Alle 7 read-only Endpoints sind **pure Passthrough** ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â der Router fÃƒÂƒÃ‚Â¼gt keine Transformation, kein Enrichment und kein Fallback-Handling hinzu. Die `execution_enabled=False`- und `write_back_allowed=False`-Invarianten kommen aus den Backing-MCP-Funktionen (nicht vom Router). |
| I-325 | `POST /operator/trading-loop/run-once` delegiert vollstÃƒÂƒÃ‚Â¤ndig an `mcp_server.run_trading_loop_once(...)`. Ein `ValueError` aus dem MCP-Layer (z.B. bei `mode=live`) wird als HTTP 400 an den Aufrufer weitergegeben. Der Router setzt keinen eigenen Mode-Guard. |
| I-326 | `auto_loop_enabled=False` im Response von `GET /operator/trading-loop/status` ist eine Strukturinvariante von `LoopStatusSummary` (I-301). Der Router setzt dieses Feld nicht ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â es kommt aus `get_trading_loop_status()`. |
| I-327 | Explizit verbotene Pfade im Operator-Router: `/operator/trade`, `/operator/execute`, `/operator/order`, `/operator/fill`, `/operator/broker`, `/operator/live`. Diese Pfade dÃƒÂƒÃ‚Â¼rfen NIEMALS registriert werden. `test_no_trading_routes` verifiziert dies. |
...
| I-329 | `tests/unit/test_api_operator.py` (13 Tests, alle passing) ist die kanonische Implementierungsreferenz fÃƒÂƒÃ‚Â¼r Sprint 43. `tests/unit/test_operator_api.py` (9 Tests, 8 failing) ist eine stale-spec-Datei aus der ursprÃƒÂƒÃ‚Â¼nglichen ÃƒÂ‚Ã‚Â§54-Planung und beschreibt Endpunkte die nicht implementiert wurden. |
| I-330 | Kein Operator-Endpoint darf `execution_enabled=True` zurÃƒÂƒÃ‚Â¼ckgeben. Kein Operator-Endpoint darf `write_back_allowed=True` zurÃƒÂƒÃ‚Â¼ckgeben. Diese Invarianten sind strukturell (backing MCP-Funktionen setzen sie) ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein Enforcement auf Ebene des Routers nÃƒÂƒÃ‚Â¶tig. |


## Sprint 44 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Operator API Hardening & Request Governance (I-331ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-340)
...
| I-332 | Der Client KANN eine `request_id` via `X-Request-Id` Request-Header vorgeben. Wenn der Wert ein valides UUID4-Format hat, wird er uebernommen. Ungueltige oder fehlende Header-Werte ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ server-generiertes UUID4. Die Entscheidung erfolgt in `get_request_id()` (Dependency). |
| I-333 | `artifacts/operator_api_audit.jsonl` ist das kanonische Operator-API-Audit-Log. Es wird fuer JEDEN `/operator/*`-Request geschrieben, der die Auth-Pruefung bestanden hat. Audit-Fehler sind nicht fatal ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â sie werden auf WARNING geloggt. Das Log enthaelt keine Secrets, Tokens oder Credentials. |
| I-334 | `POST /operator/trading-loop/run-once` akzeptiert optionalen `X-Idempotency-Key` Header. Wenn gesetzt und bereits im In-memory-Buffer gesehen ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ HTTP 409 mit `error="duplicate_idempotency_key"`. Wenn nicht gesetzt ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ kein Idempotency-Check. Der Idempotency-Buffer ist kein Scheduler und startet keine Zyklen. |
...
| I-336 | ALLE Fehlerantworten aus `/operator/*` MUESSEN der kanonischen Error-Shape folgen: `{"error": "<code>", "detail": "<human>", "request_id": "<uuid>"}`. Kein unstrukturierter Error-String. Die `error`-Codes sind in ÃƒÂ‚Ã‚Â§55.5 kanonisch definiert. |
...
| I-339 | HTTP 500 aus `/operator/*` darf niemals einen nackten Stacktrace oder eine unstrukturierte Fehlermeldung zurueckgeben. Jede unbehandelte Exception ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `{"error": "internal_error", "detail": "<sanitized>", "request_id": "<uuid>"}` mit HTTP 500. `request_id` ist auch bei 500 verpflichtend (falls generiert). |
...
> **Sprint 44C (2026-03-22):** I-331ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-340 (oben) wurden auf Basis des ÃƒÂ‚Ã‚Â§55-Entwurfs geschrieben und enthalten Drift. Kanonische, implementierungsbasierte Korrekturen:

| ID | Invariant (kanonisch, Sprint 44C) |
|---|---|
| I-331C | Jeder `/operator/*`-Response enthÃƒÂƒÃ‚Â¤lt `X-Request-ID` und `X-Correlation-ID` als Response-Headers. `request_id` hat das Format `req_<uuid4_hex>` (nicht UUID4). Client KANN via `X-Request-ID` Request-Header eigene ID vorgeben (validiert via `^[A-Za-z0-9._:-]{1,128}$`). |
...
| I-333C | Das Audit-Log ist `artifacts/operator_api_guarded_audit.jsonl` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â ausschliesslich fÃƒÂƒÃ‚Â¼r `POST /operator/trading-loop/run-once` (guarded POST), NICHT fÃƒÂƒÃ‚Â¼r alle Operator-Requests. Felder: event, endpoint, request_id, correlation_id, idempotency_key, outcome, error_code, idempotency_replayed, mode, symbol, provider, execution_enabled=false, write_back_allowed=false. |
| I-334C | `Idempotency-Key` Header (nicht `X-Idempotency-Key`) ist **REQUIRED** fÃƒÂƒÃ‚Â¼r `POST /operator/trading-loop/run-once`. Fehlt er ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ HTTP 400 `missing_idempotency_key`. Replay-Verhalten: gleicher Key + gleicher SHA256-Fingerprint ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ stored Response zurÃƒÂƒÃ‚Â¼ck, keine NeuausfÃƒÂƒÃ‚Â¼hrung. Gleicher Key + anderer Fingerprint ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ HTTP 409 `idempotency_key_conflict`. |
| I-335C | Der Idempotency-Buffer ist `OrderedDict[str, _IdempotencyRecord]` (maxlen=256, FIFO, Thread-safe mit Lock). `_IdempotencyRecord` enthÃƒÂƒÃ‚Â¤lt `request_fingerprint` und `response_payload`. Ein Replay setzt `idempotency_replayed=True`. Replay-Requests zÃƒÂƒÃ‚Â¤hlen NICHT gegen den Rate-Limiter. |
...
| I-337C | Sliding-Window Rate-Limiter fÃƒÂƒÃ‚Â¼r `POST /operator/trading-loop/run-once`: 5 Requests pro 30 Sekunden pro `operator_subject` (= `token_<sha256[:16]>` des Bearer-Tokens). ÃƒÂƒÃ…Â“berschreitung ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ HTTP 429 `guarded_rate_limited`. In-memory (`deque[float]`), nicht persistent, Thread-safe (Lock). |
| I-338C | `_append_guarded_audit()` ist never-raise: `OSError` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ silent `return`, keine Caller-sichtbare Exception. Der `_WORKSPACE_ROOT` Pfad ist via `Path(__file__).resolve().parents[3]` gesetzt ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â testbar via `monkeypatch.setattr(operator_router, "_WORKSPACE_ROOT", tmp_path)`. |
| I-339C | `_resolve_read_payload()` ist der kanonische Wrapper fÃƒÂƒÃ‚Â¼r alle read-only Endpoints. Er setzt Context-Headers und fÃƒÂƒÃ‚Â¤ngt JEDE Exception ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ HTTP 503 mit endpoint-spezifischem Error-Code (z.B. `status_unavailable`, `portfolio_snapshot_unavailable`). |
| I-340C | Sprint 44 oeffnet keinen neuen Live-, Broker- oder Execution-Pfad. `_reset_operator_guard_state_for_tests()` ist eine Test-Helper-Funktion ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â sie leert `_IDEMPOTENCY_CACHE` und `_GUARDED_RATE_LIMIT_BUCKETS` fÃƒÂƒÃ‚Â¼r deterministische Unit-Tests. |


---

## Sprint 45 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â S45_OPERATOR_USABILITY_BASELINE (2026-03-22)
...
| I-342 | `execution_enabled=false` und `write_back_allowed=false` sind invariante Felder in JEDEM `daily_operator_summary`-Response ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â unabhaengig vom Aggregations-Ergebnis. |
...
| I-345 | Die CLI-Ausgabe von `trading-bot research daily-summary` ist menschenlesbar ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein JSON-Dump ohne expliziten `--json`-Flag. |
...
| I-350 | Sprint 45 fuegt keinen neuen Aggregations-Layer hinzu, der parallel zum bestehenden MCP-Tool-Layer existiert. Der Daily Operator View ist eine Aggregations-Fassung bestehender Tools ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein zweiter Stack. |



---

> **Sprint 45C (2026-03-22):** I-341ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€ÂœI-350 (oben) basieren auf dem ÃƒÂ‚Ã‚Â§56-Entwurf.
...
| I-343C | Aggregation ist best-effort via `_safe_daily_surface_load`: Exception in einem Sub-Tool ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `None` zurueck ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `build_daily_operator_summary` erhaelt `None` fuer diese Sektion ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ Fallback-Werte. Das `sources`-Feld enthaelt nur Sub-Tool-Namen die erfolgreich `dict`-Payload geliefert haben. |
...
| I-347C | Telegram `/daily_summary` delegiert an `self._get_daily_operator_summary` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ `mcp_server.get_daily_operator_summary`. Format: Markdown mit `_inline`-escaped Werten. Kein JSON-Dump. Kein eigenstaendiger Aggregations-Pfad. |
...
## Sprint 46 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â S46_OPERATOR_DASHBOARD_BASELINE (2026-03-22)
...
| I-354 | Fail-closed: leeres `APP_API_KEY` ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ HTTP 503. Das Dashboard erfordert keinen Bearer-Token vom Browser, prueft aber ob ein API-Key konfiguriert ist. |
| I-355 | Auto-Refresh via `<meta http-equiv="refresh" content="60">`. Kein JavaScript-Polling. |
| I-356 | Bei Exception in `get_daily_operator_summary` gibt das Dashboard eine HTML-Fehlerseite aus (Status "unavailable") ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein Python-Stack-Trace, kein HTTP 500 mit JSON-Body. |
...
| I-359 | Farb-Konvention (CSS-only): `readiness_status == "ok"` gruen, `"warning"` orange, alles andere rot. Nur fuer visuelle Schnellorientierung ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein funktionaler Unterschied. |
...
## S46C ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Dashboard Path Freeze (2026-03-22)

### Canonical Runtime Path (einzig gÃƒÂƒÃ‚Â¼ltig)
...
- `/static/dashboard.html` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â **existiert nicht**, wurde nie implementiert, ist S46 Out-of-Scope
- `StaticFiles`-Mount ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â **nicht vorhanden** in `app/api/main.py`
- Jinja2-Template ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â **nicht installiert**, nicht verwendet

### Teststand-Drift ErklÃƒÂƒÃ‚Â¤rung
...
| S46C freeze | **1503 passed, ruff clean** | Einzige gÃƒÂƒÃ‚Â¼ltige Referenz ab jetzt |

### S46C Invariant-ErgÃƒÂƒÃ‚Â¤nzungen
...
| I-363 | `app/security/auth.py` whitelistet `/dashboard` und `/dashboard/` ohne Bearer-Token ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein Browserzwang. Kein anderer Dashboard-Pfad ist whitelistet. |

## S47 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Drilldown & History Invarianten (I-364..I-368)
...
| I-366 | Beide Endpoints erfordern Bearer-Auth ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein Auth-Whitelist-Eintrag |
| I-367 | `execution_enabled: false`, `write_back_allowed: false` in allen neuen Responses |
| I-368 | Kanonische Drilldown-Kette (ÃƒÂ‚Ã‚Â§59.5) ist implementiert ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein zweiter Daily-Aggregat-Pfad |
...
```
GET /operator/daily-summary      ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ Tageseinstieg
GET /dashboard                   ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ Visueller Ueberblick
  ÃƒÂ¢Ã¢Â€Â Ã¢Â€Âœ
GET /operator/readiness          ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ Issues-Liste
GET /operator/decision-pack      ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ Blocking-Decisions
...
GET /operator/exposure-summary
  ÃƒÂ¢Ã¢Â€Â Ã¢Â€Âœ (neu S47)
GET /operator/review-journal     ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ Review-Journal-Historie
GET /operator/resolution-summary ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ Per-Source-AuflÃƒÂƒÃ‚Â¶sungsstatus
```

## S48 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Operator Surface Completion Invarianten (I-369..I-373)

Status: CLOSED (S48 completed, baseline 1515 passed / ruff clean).

| # | Invariant |
|---|---|
| I-369 | `/resolution` Telegram delegiert an `mcp_server.get_resolution_summary()` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein eigenes Aggregat |
| I-370 | `/decision_pack` Telegram delegiert an `mcp_server.get_decision_pack_summary()` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein eigenes Aggregat |
...
| I-373 | Surface Completion Matrix (ÃƒÂ‚Ã‚Â§61.2) ist autoritativ fÃƒÂƒÃ‚Â¼r Surface-ParitÃƒÂƒÃ‚Â¤t |
...
|---|---|---|---|---|---|---|---|---|
| **API** | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| **CLI** | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ |
| **Telegram** | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | n/a |
| **Dashboard** | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦(visual) | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦(status) | n/a | n/a | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦ | ÃƒÂ¢Ã…Â“Ã¢Â€Â¦(last) |

## S48B ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Scope-Freeze-Invarianten

| # | Invariant |
|---|---|
| I-374 | ÃƒÂ‚Ã‚Â§61 ist der einzige bindende S48-Contract. Kein frÃƒÂƒÃ‚Â¼herer Entwurf (Dashboard-Subpages) ist fÃƒÂƒÃ‚Â¼r S48 gÃƒÂƒÃ‚Â¼ltig. |
...
## S49 ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Alerting/Digest Baseline Invarianten (I-378..I-383)

Status: ACTIVE.

| # | Invariant |
|---|---|
| I-378 | `get_alert_audit_summary()` liest nur `alert_audit.jsonl` ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein neues Aggregat, keine Pipeline-ÃƒÂƒÃ¢Â€Âžnderung |
| I-379 | `GET /operator/alert-audit` erfordert Bearer-Auth ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â kein Whitelist-Eintrag |
| I-380 | `execution_enabled=False`, `write_back_allowed=False` in allen S49-Responses |
| I-381 | `ALERT_DRY_RUN=true` bleibt Standard ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â S49 verÃƒÂƒÃ‚Â¤ndert keine Alert-Trigger |
| I-382 | `DailyOperatorSummary` und `build_daily_operator_summary` werden in S49 nicht verÃƒÂƒÃ‚Â¤ndert (frozen) |
| I-383 | Kein Digest-Scheduling, kein Cron-Trigger, kein Auto-Dispatch in S49 |

## S50A ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Canonical Path Inventory Invarianten (I-384..I-389)
...
| I-386 | FÃƒÂƒÃ‚Â¼r jede Operator-Surface wird eine eindeutige Klassifikation gefÃƒÂƒÃ‚Â¼hrt: `canonical`, `alias`, `superseded`, `provisional`. |
| I-387 | `CANONICAL_SURFACE_INVENTORY.md` ist die S50A-Source-of-Truth fÃƒÂƒÃ‚Â¼r Surface-Klassifikation und Implementierungsreferenzen. |
...
| I-392 | Das provisional CLI set ist als primÃƒÂƒÃ‚Â¤rer Entscheidungsgegenstand fuer den Freeze zu behandeln. |
...
Bestehende Pipeline bleibt unverÃƒÂƒÃ‚Â¤ndert:
```
pipeline/service.py ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ AlertService.process_document() ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ alert_audit.jsonl
                    ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ AlertService.send_digest() [manuell, via CLI/API/test]
```


---

## Phase-3 / S50A ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Architecture Stability Note (2026-03-22)

The three-tier intelligence architecture (Rule ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ Local ÃƒÂ¢Ã¢Â€Â Ã¢Â€Â™ External LLM) is stable and unchanged.
...
The inventory covers operator-facing runtime surfaces (MCP, API, CLI, Telegram, Dashboard) ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â
...
- Tier 2 (internal companion model): aspirational ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Sprint 5 planned, not yet implemented
...
## Phase-4 / PH4A ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â Signal Quality Audit Context (2026-03-22)

Phase 4 opens with `PH4A_SIGNAL_QUALITY_AUDIT_BASELINE`. The three-tier stack is the primary subject of the audit.
Contract and acceptance freeze is completed in `docs/contracts.md` ÃƒÂ‚Ã‚Â§67; execution proceeds on the frozen metric/data-slice/output set only.
...
- Tier 2 (internal companion model) remains aspirational ÃƒÂ¢Ã¢Â‚Â¬Ã¢Â€Â not a PH4A target.
