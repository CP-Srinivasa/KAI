...
| ph4h_status | `active (definition) — policy review on rule-only ceiling and fallback actionability` |
...
| I-303 | `run_trading_loop_once()` defaults to `provider="mock"`, which instantiates `MockMarketDataAdapter`. No real external adapter may be used in the default run-once path. The default `analysis_profile="conservative"` generates a non-actionable `AnalysisResult` — this intentionally produces a `CycleStatus.NO_SIGNAL` cycle without paper order creation. |
...
| I-305 | `run_trading_loop_once()` creates a fresh `PaperExecutionEngine` per call (no portfolio replay from `paper_execution_audit.jsonl`). If a cycle completes with a simulated fill, the `PaperExecutionEngine` writes the fill to `execution_audit_path` (default: `artifacts/paper_execution_audit.jsonl`). This is correct behavior — run-once paper fills ARE part of the paper execution audit. |
...
| I-309 | `research trading-loop-run-once` CLI command MUST be registered (it is already declared in `FINAL_RESEARCH_COMMAND_NAMES`). Until registered, `test_research_command_inventory_matches_registration_and_help` fails — this is the Sprint 41 implementation blocker. Once registered, the test MUST pass. |
...
## Sprint 42+42C — Telegram Webhook Hardening (I-311–I-320) ✅

> **Sprint 42C (2026-03-21)**: Invarianten korrigiert auf tatsächliche Implementierung. I-311/I-312/I-313/I-315/I-316/I-318/I-319/I-320 wurden mit falschen Namen geschrieben und sind hier finalisiert.

| ID | Invariant |
|---|---|
| I-311 | Webhook-Hardening ist **vollständig in `app/messaging/telegram_bot.py` integriert** (kein separates Modul). `TelegramWebhookProcessResult` (frozen dataclass: `accepted`, `processed`, `rejection_reason`, `update_id`, `update_type`) ist das kanonische Resultat-Modell. Kein separates Legacy-Validator-/Webhook-Modul. |
...
| I-313 | `webhook_secret_token=None/""` at construction → `rejection_reason="webhook_secret_not_configured"` on every request — fail-closed. The `webhook_configured` property MUST be checked before processing. `webhook_signature_required: True` in runtime config is satisfied when `webhook_secret_token` is non-empty. |
| I-314 | Secret-Token mismatch → `rejection_reason="invalid_secret_token"`. Missing header → `rejection_reason="missing_secret_token_header"`. Secret comparison MUST use `hmac.compare_digest` (constant-time). Implemented as `_constant_time_secret_match()`. |
...
| I-316 | Replay deduplication uses an in-memory `OrderedDict` with FIFO eviction at `maxlen=2048` (not `deque`). A duplicate `update_id` → `rejection_reason="duplicate_update_id"`. Buffer is NOT persisted. Restart = empty buffer. This is safe: Telegram confirms delivery on HTTP 200. |
...
| I-318 | Rejected webhook requests MUST produce exactly one audit append to `artifacts/telegram_webhook_rejections.jsonl` (rejections only — not all requests). Accepted requests are covered by the existing `artifacts/operator_commands.jsonl`. No secrets or credentials in the audit. |
...
## Sprint 43+43C — FastAPI Operator API Surface (I-321–I-330) ✅

> **Sprint 43C (2026-03-21):** Invarianten basieren auf tatsächlicher Implementierung in `app/api/routers/operator.py`. Sprint-43-Entwurf hatte andere Endpunkt-Namen und Webhook-Endpoints die nicht implementiert wurden.

| ID | Invariant |
|---|---|
| I-321 | `app/api/routers/operator.py` ist das kanonische Operator-API-Modul. Es enthält **keine eigene Business-Logik** — ausschliesslich Delegation an bestehende MCP-Funktionen (`mcp_server.*`). Kein Datenbankzugriff, kein direkter Adapter-Zugriff, kein Scheduling. |
| I-322 | `require_operator_api_token` ist eine FastAPI-Dependency-Funktion (DI), gesetzt als `dependencies=[Depends(require_operator_api_token)]` auf dem gesamten Router. Sie ist NICHT über die bestehende `app/security/auth.py`-Bearer-Middleware implementiert. Leer `APP_API_KEY` → 503. Kein Header → 401. Falscher Token → 403. Tokenvergleich via `secrets.compare_digest` (constant-time). |
| I-323 | `GET /operator/status` und `GET /operator/readiness` sind **Aliases** — beide rufen `mcp_server.get_operational_readiness_summary()` ohne weitere Parameter auf. Kein inhaltlicher Unterschied. Beide sind read_only. |
| I-324 | Alle 7 read-only Endpoints sind **pure Passthrough** — der Router fügt keine Transformation, kein Enrichment und kein Fallback-Handling hinzu. Die `execution_enabled=False`- und `write_back_allowed=False`-Invarianten kommen aus den Backing-MCP-Funktionen (nicht vom Router). |
| I-325 | `POST /operator/trading-loop/run-once` delegiert vollständig an `mcp_server.run_trading_loop_once(...)`. Ein `ValueError` aus dem MCP-Layer (z.B. bei `mode=live`) wird als HTTP 400 an den Aufrufer weitergegeben. Der Router setzt keinen eigenen Mode-Guard. |
| I-326 | `auto_loop_enabled=False` im Response von `GET /operator/trading-loop/status` ist eine Strukturinvariante von `LoopStatusSummary` (I-301). Der Router setzt dieses Feld nicht — es kommt aus `get_trading_loop_status()`. |
| I-327 | Explizit verbotene Pfade im Operator-Router: `/operator/trade`, `/operator/execute`, `/operator/order`, `/operator/fill`, `/operator/broker`, `/operator/live`. Diese Pfade dürfen NIEMALS registriert werden. `test_no_trading_routes` verifiziert dies. |
...
| I-329 | `tests/unit/test_api_operator.py` (13 Tests, alle passing) ist die kanonische Implementierungsreferenz für Sprint 43. `tests/unit/test_operator_api.py` (9 Tests, 8 failing) ist eine stale-spec-Datei aus der ursprünglichen §54-Planung und beschreibt Endpunkte die nicht implementiert wurden. |
| I-330 | Kein Operator-Endpoint darf `execution_enabled=True` zurückgeben. Kein Operator-Endpoint darf `write_back_allowed=True` zurückgeben. Diese Invarianten sind strukturell (backing MCP-Funktionen setzen sie) — kein Enforcement auf Ebene des Routers nötig. |


## Sprint 44 — Operator API Hardening & Request Governance (I-331–I-340)
...
| I-332 | Der Client KANN eine `request_id` via `X-Request-Id` Request-Header vorgeben. Wenn der Wert ein valides UUID4-Format hat, wird er uebernommen. Ungueltige oder fehlende Header-Werte → server-generiertes UUID4. Die Entscheidung erfolgt in `get_request_id()` (Dependency). |
| I-333 | `artifacts/operator_api_audit.jsonl` ist das kanonische Operator-API-Audit-Log. Es wird fuer JEDEN `/operator/*`-Request geschrieben, der die Auth-Pruefung bestanden hat. Audit-Fehler sind nicht fatal — sie werden auf WARNING geloggt. Das Log enthaelt keine Secrets, Tokens oder Credentials. |
| I-334 | `POST /operator/trading-loop/run-once` akzeptiert optionalen `X-Idempotency-Key` Header. Wenn gesetzt und bereits im In-memory-Buffer gesehen → HTTP 409 mit `error="duplicate_idempotency_key"`. Wenn nicht gesetzt → kein Idempotency-Check. Der Idempotency-Buffer ist kein Scheduler und startet keine Zyklen. |
...
| I-336 | ALLE Fehlerantworten aus `/operator/*` MUESSEN der kanonischen Error-Shape folgen: `{"error": "<code>", "detail": "<human>", "request_id": "<uuid>"}`. Kein unstrukturierter Error-String. Die `error`-Codes sind in §55.5 kanonisch definiert. |
...
| I-339 | HTTP 500 aus `/operator/*` darf niemals einen nackten Stacktrace oder eine unstrukturierte Fehlermeldung zurueckgeben. Jede unbehandelte Exception → `{"error": "internal_error", "detail": "<sanitized>", "request_id": "<uuid>"}` mit HTTP 500. `request_id` ist auch bei 500 verpflichtend (falls generiert). |
...
> **Sprint 44C (2026-03-22):** I-331–I-340 (oben) wurden auf Basis des §55-Entwurfs geschrieben und enthalten Drift. Kanonische, implementierungsbasierte Korrekturen:

| ID | Invariant (kanonisch, Sprint 44C) |
|---|---|
| I-331C | Jeder `/operator/*`-Response enthält `X-Request-ID` und `X-Correlation-ID` als Response-Headers. `request_id` hat das Format `req_<uuid4_hex>` (nicht UUID4). Client KANN via `X-Request-ID` Request-Header eigene ID vorgeben (validiert via `^[A-Za-z0-9._:-]{1,128}$`). |
...
| I-333C | Das Audit-Log ist `artifacts/operator_api_guarded_audit.jsonl` — ausschliesslich für `POST /operator/trading-loop/run-once` (guarded POST), NICHT für alle Operator-Requests. Felder: event, endpoint, request_id, correlation_id, idempotency_key, outcome, error_code, idempotency_replayed, mode, symbol, provider, execution_enabled=false, write_back_allowed=false. |
| I-334C | `Idempotency-Key` Header (nicht `X-Idempotency-Key`) ist **REQUIRED** für `POST /operator/trading-loop/run-once`. Fehlt er → HTTP 400 `missing_idempotency_key`. Replay-Verhalten: gleicher Key + gleicher SHA256-Fingerprint → stored Response zurück, keine Neuausführung. Gleicher Key + anderer Fingerprint → HTTP 409 `idempotency_key_conflict`. |
| I-335C | Der Idempotency-Buffer ist `OrderedDict[str, _IdempotencyRecord]` (maxlen=256, FIFO, Thread-safe mit Lock). `_IdempotencyRecord` enthält `request_fingerprint` und `response_payload`. Ein Replay setzt `idempotency_replayed=True`. Replay-Requests zählen NICHT gegen den Rate-Limiter. |
...
| I-337C | Sliding-Window Rate-Limiter für `POST /operator/trading-loop/run-once`: 5 Requests pro 30 Sekunden pro `operator_subject` (= `token_<sha256[:16]>` des Bearer-Tokens). Überschreitung → HTTP 429 `guarded_rate_limited`. In-memory (`deque[float]`), nicht persistent, Thread-safe (Lock). |
