...
| archive | `docs/contracts_archive.md` (closed Â§Â§38â€“Â§82) |
...
| [Archive](contracts_archive.md) | Closed Â§Â§38â€“Â§82 (Phase 1â€“4) |
...
@dataclass
class FetchItem:
    url: str                        # required Ã¢â‚¬â€ canonical item URL
...
Rules:
- **No analysis** Ã¢â‚¬â€ no scores, no sentiment, no priority, no tickers, no entity mentions
- **No persistence state** Ã¢â‚¬â€ no `status`, `is_analyzed`, `is_duplicate`, `content_hash`, `id`
- **No source metadata** Ã¢â‚¬â€ `source_id`, `source_name`, `source_type` are injected by `normalize_fetch_item()`, never by the adapter
- As close to the source as possible Ã¢â‚¬â€ minimal transformation before `normalize_fetch_item()`
...
Normalization is adapter-owned Ã¢â‚¬â€ it must NOT move into `persist_fetch_result()`, which is a
...
class FetchResult:
    source_id: str
    documents: list[CanonicalDocument]  # never None Ã¢â‚¬â€ empty list on failure
    fetched_at: datetime
    success: bool
...
Rules:
- adapter must never raise Ã¢â‚¬â€ catch all exceptions internally
...
- `content_hash` must not be set by adapter Ã¢â‚¬â€ auto-computed by `CanonicalDocument`
...
class CanonicalDocument(BaseModel):
    id: UUID                            # primary key Ã¢â‚¬â€ never change after persist
    url: str                            # required Ã¢â‚¬â€ dedup key
    title: str                          # required
    ...
    content_hash: str | None = None     # auto-computed Ã¢â‚¬â€ never set manually
    status: DocumentStatus              # lifecycle state Ã¢â‚¬â€ see below
...
Rules:
- `content_hash` is auto-computed from `url|title|raw_text` Ã¢â‚¬â€ never set manually
- `word_count` is a `@computed_field` Ã¢â‚¬â€ never stored in DB
...
class AnalysisResult(BaseModel):
    ...
    relevance_score: float              # [0.0, 1.0] Ã¢â‚¬â€ blended with keyword hits by apply_to_document()
    impact_score: float                 # [0.0, 1.0]
    novelty_score: float                # [0.0, 1.0]
    confidence_score: float             # [0.0, 1.0] Ã¢â‚¬â€ in-memory only, NOT persisted to DB
    ...
    explanation_short: str              # required Ã¢â‚¬â€ concise reasoning
    explanation_long: str               # required Ã¢â‚¬â€ full reasoning
...
Rules:
- Must be fully populated Ã¢â‚¬â€ all score fields are required (no optional scores)
- Must be schema-validated Ã¢â‚¬â€ all ranges enforced by Pydantic
...
- `spam_probability` IS stored on `AnalysisResult` for audit Ã¢â‚¬â€ but scoring functions
  (`compute_priority`, `is_alert_worthy`) receive it as an **explicit separate parameter**
- `recommended_priority` is set by `apply_to_document()` after `compute_priority()` runs Ã¢â‚¬â€ not by the LLM
- `AnalysisResult` is in-memory only Ã¢â‚¬â€ no separate DB table
...
```
pending Ã¢â€ â€™ persisted Ã¢â€ â€™ analyzed
         Ã¢â€ Ëœ failed
         Ã¢â€ Ëœ duplicate
```
...
|---|---|---|
| `pending` | in-memory only Ã¢â‚¬â€ not yet saved to DB | `prepare_ingested_document()` in `document_ingest.py` |
...
| `failed` | non-recoverable error Ã¢â‚¬â€ kept for audit | `repo.update_status(FAILED)` Ã¢â‚¬â€ ingest, `run_rss_pipeline()`, and `analyze_pending` CLI error handlers |
| `duplicate` | blocked at dedup gate Ã¢â‚¬â€ NOT saved | detected in-memory; `repo.mark_duplicate()` for retroactive marking |
...
Rules:
- transitions are one-way Ã¢â‚¬â€ no rollback, no recycling
...
| Boundary | Rule |
|---|---|
| Ingestion Ã¢â€ â€™ Storage | adapter returns `FetchResult`; only `persist_fetch_result()` persists |
| Storage Ã¢â€ â€™ Analysis | `repo.get_pending_documents()` feeds the analysis queue Ã¢â‚¬â€ filters `status=PERSISTED` (not just flags) |
| Analysis Ã¢â€ â€™ Storage | `apply_to_document()` then `repo.update_analysis()` Ã¢â‚¬â€ no other path |
| Analysis Ã¢â€ â€™ Alerting | `is_alert_worthy()` is the only gate Ã¢â‚¬â€ no direct score access |
| LLM calls | always via `BaseAnalysisProvider.analyze()` Ã¢â‚¬â€ never direct SDK calls |
| Config | always via `AppSettings` Ã¢â‚¬â€ never `os.environ` directly |
...
priority = round(raw Ãƒâ€” 9) + 1          # maps [0.0, 1.0] Ã¢â€ â€™ [1, 10]
...
Cap: if `spam_probability > 0.7` Ã¢â€ â€™ `priority = min(priority, 3)` (applied after bonus)
...
Rules:
- `run()` input is always `CanonicalDocument` Ã¢â‚¬â€ never a raw dict or ORM model
- `run()` output is always `PipelineResult` Ã¢â‚¬â€ never raises (errors surfaced in `result.error`)
...
Scoring is part of the pipeline result Ã¢â‚¬â€ not a separate side-effect.
...
3. title hash match (score 0.85 Ã¢â‚¬â€ catches same headline across sources)

Rules:
- conservative by default Ã¢â‚¬â€ prefer false negatives over false positives
- `is_duplicate()` never writes to DB Ã¢â‚¬â€ read-only
- dedup is enforced exclusively by `document_ingest.py` before `repo.save_document()`
- `filter_scored()` is used by `persist_fetch_result()` Ã¢â‚¬â€ returns all docs with scores for auditing
...
Rules:
- every provider must return a fully validated `LLMAnalysisOutput` Ã¢â‚¬â€ never a raw dict (I-7)
- `analyze()` never receives a `CanonicalDocument` directly Ã¢â‚¬â€ caller extracts `title` + `text`
...
- structured output enforcement is provider-specific (OpenAI: `response_format`, Anthropic: tool-use,
  Gemini: `response_schema`) Ã¢â‚¬â€ but the output contract is identical
...
- never trust provider output blindly Ã¢â‚¬â€ schema validation is mandatory

---

### 11. Sprint 4 Ã¢â‚¬â€ Research & Signal Contracts

These contracts define the Sprint 4 output layer. All three types are **in-memory only** Ã¢â‚¬â€
...
Rules:
- Source: `monitor/watchlists.yml` Ã¢â‚¬â€ loaded via `WatchlistEntry` + `load_watchlist()`
...
- `WatchlistRegistry` is read-only after construction Ã¢â‚¬â€ no mutations during runtime
- `load_watchlist()` returns `[]` (not an error) if the file does not exist
- `find_by_text()` Ã¢â‚¬â€ Sprint 4B planned, not yet implemented; use `filter_documents()` instead
...
class BriefDocument(BaseModel):
    document_id: str          # str(CanonicalDocument.id) Ã¢â‚¬â€ traceability
    title: str
    url: str
...
Rules:
- Input: `list[CanonicalDocument]` Ã¢â‚¬â€ only `is_analyzed=True` docs are used
- `ResearchBriefBuilder.build()` never raises Ã¢â‚¬â€ returns empty brief on empty/unanalyzed input
- `_ACTIONABLE_PRIORITY_THRESHOLD = 8` Ã¢â‚¬â€ must stay in sync with `ThresholdEngine.min_priority`
...
- `ResearchBrief` is in-memory only Ã¢â‚¬â€ no DB table, no persistence
...
class SignalCandidate(BaseModel):
    ...
    signal_id: str              # f"sig_{document_id}" Ã¢â‚¬â€ deterministic
    document_id: str            # str(CanonicalDocument.id) Ã¢â‚¬â€ traceability

    target_asset: str           # primary asset ("BTC", "ETH", "General Market")
    direction_hint: str         # "bullish" | "bearish" | "neutral"
                                # NEVER "buy" / "sell" / "hold" Ã¢â‚¬â€ not an execution instruction
    confidence: float           # proxy: doc.relevance_score Ã¢â‚¬â€ [0.0, 1.0]
    supporting_evidence: str    # doc.summary or doc.title
    contradicting_evidence: str # static note Ã¢â‚¬â€ not extracted in primary scan
    risk_notes: str             # spam_prob + market_scope metadata
    source_quality: float       # doc.credibility_score Ã¢â‚¬â€ [0.0, 1.0]
    recommended_next_step: str  # always ends with "Ã¢â‚¬â€ human decision required."
...
Rules:
- `priority >= 8` is a hard constraint Ã¢â‚¬â€ Pydantic `Field(ge=8)` enforced at construction
- `direction_hint` is research language, NOT trading instruction Ã¢â‚¬â€ "bullish"/"bearish"/"neutral"
- `signal_id` is deterministic: `f"sig_{document_id}"` Ã¢â‚¬â€ idempotent for same document
- `watchlist_boosts`: `{"BTC": 1}` raises effective priority by 1 for watchlist assets;
  capped at 10; never raises above 10
- `confidence_score` from `AnalysisResult` is NOT persisted to DB Ã¢â‚¬â€ `relevance_score` is
  used as the confidence proxy (available in DB)
- `SignalCandidate` is in-memory only Ã¢â‚¬â€ no DB table, no persistence
- `extract_signal_candidates()` never raises Ã¢â‚¬â€ returns `[]` if no candidates qualify
...
| No DB writes | `ResearchBrief` and `SignalCandidate` are always in-memory Ã¢â‚¬â€ never persisted |
| No LLM calls | Research layer is pure computation Ã¢â‚¬â€ no provider calls, no external I/O |
| Watchlist source | Always from `monitor/watchlists.yml` via `WatchlistRegistry.from_monitor_dir()` |
| CLI entry point | `research` Typer subgroup Ã¢â‚¬â€ `watchlists`, `brief`, `signals` commands |
...
`EnsembleProvider` (`app/analysis/ensemble/provider.py`) is not a factory target Ã¢â‚¬â€ it wraps
multiple providers directly. Its `provider_name` is a compound string like
`"ensemble(openai,internal)"` (see Ã‚Â§13e on EnsembleProvider and analysis_source).
...
class AnalysisSource(StrEnum):
    RULE = "rule"                  # Tier 1 Ã¢â‚¬â€ fallback / rule-based heuristics
    INTERNAL = "internal"          # Tier 2 Ã¢â‚¬â€ InternalModelProvider or InternalCompanionProvider
    EXTERNAL_LLM = "external_llm"  # Tier 3 Ã¢â‚¬â€ OpenAI / Anthropic / Gemini
```
...
```python
# app/core/domain/document.py Ã¢â‚¬â€ compatibility accessor
...
- Distillation corpus selects ONLY `analysis_source=EXTERNAL_LLM` documents (Ã‚Â§14e)
...
field (Ã‚Â§14c). The internal reasoning trace is not part of the training corpus output format.
...
1. `_resolve_analysis_source()` in `app/analysis/pipeline.py` maps any `provider_name` that
   starts with `"ensemble("` to `INTERNAL`. This is the primary guard Ã¢â‚¬â€ it sets
...
```json
{
  "messages": [
    ...
    {"role": "assistant", "content": "<JSON target scores Ã¢â‚¬â€ sorted keys>"}
  ],
  "metadata": {
    "document_id":     "<uuid Ã¢â‚¬â€ str>",
...
#### 14c. `co_thought` Ã¢â‚¬â€ Final Decision: REMOVED
...
1. **Contamination risk**: Rule-based analysis sets `explanation_short = "Rule-based fallback
   analysis. ..."` Ã¢â‚¬â€ a heuristic label, not reasoning. Including it as chain-of-thought
...
3. **Schema coupling**: `co_thought` would couple the export to `doc.metadata["explanation_short"]`
   Ã¢â‚¬â€ an implementation detail, not a stable contract field.
...
# Ã¢â€ â€™ returns doc.analysis_source if explicitly set (Sprint 5B field)
# Ã¢â€ â€™ falls back to derivation from doc.provider (legacy path for pre-Sprint-5B rows)
```

The export reads `doc.effective_analysis_source.value` Ã¢â‚¬â€ never derives analysis_source inline.
...
- `doc.provider` Ã¢â‚¬â€ may be `"openai"` (teacher), `"internal"` (not teacher), or a legacy
  composite `"ensemble(openai,internal)"` Ã¢â‚¬â€ ambiguous without `analysis_source`
- `doc.metadata["ensemble_chain"]` Ã¢â‚¬â€ audit trail only, not a classification signal
...
#### 14f. provider vs analysis_source Ã¢â‚¬â€ Contract Separation

These are two distinct concepts that must never be conflated:

| Concept | Field | Type | Persistence | Purpose |
|---------|-------|------|-------------|---------|
| `provider` | `doc.provider` | `str \| None` | DB column | Technical engine name. Pre-5C: `"openai"`, `"internal"`, `"ensemble(openai,internal)"`, `"fallback"`. Post-5C: always the **winner name** Ã¢â‚¬â€ never a composite string. |
| `analysis_source` | `doc.analysis_source` | `AnalysisSource` enum | DB column (migration 0006) | Semantic tier: `RULE` / `INTERNAL` / `EXTERNAL_LLM` Ã¢â‚¬â€ stable, use this for filtering. |
...
**Rules:**
- `provider` is a technical string Ã¢â‚¬â€ never use it directly for corpus filtering or tier decisions
- `analysis_source` is the stable semantic value Ã¢â‚¬â€ always use this for filtering and guardrails
- `provider` semantics changed in Sprint-5C: composite `"ensemble(...)"` strings are legacy only
- `analysis_source` must NEVER be set manually Ã¢â‚¬â€ always set by pipeline at result creation time
- Downstream code (ResearchBrief, SignalCandidate, alerts) consumes analysis results via the same
  `CanonicalDocument` contract regardless of which tier produced them Ã¢â‚¬â€ no branching on `provider`
...
- `ResearchBrief.key_documents` Ã¢â‚¬â€ Ã¢Å“â€¦ included
- `ResearchBrief.top_actionable_signals` Ã¢â‚¬â€ Ã¢Å“â€¦ if `priority >= 8`
- `SignalCandidate` Ã¢â‚¬â€ Ã¢Å“â€¦ if `priority >= 8` (companion can reach 8 with strong output)
- Alert gating Ã¢â‚¬â€ Ã¢Å“â€¦ same `ThresholdEngine.is_alert_worthy()` path
...
### 15. Sprint-5C Ã¢â‚¬â€ Winner-Traceability Contract
...
`doc.analysis_source = INTERNAL` and `doc.provider = "ensemble(openai,internal)"` Ã¢â‚¬â€
...
def _resolve_runtime_provider_name(provider: BaseAnalysisProvider | None) -> str | None:
    """Return the actual winner name after analyze() Ã¢â‚¬â€ for EnsembleProvider via duck typing."""
    if provider is None:
        return None
...
pipelines Ã¢â‚¬â€ `_resolve_runtime_provider_name` returns the winner name instead. The composite
guard remains in `CanonicalDocument.effective_analysis_source` for legacy DB rows only (Ã‚Â§15e).

**Pipeline call site** (success path):

```python
# trace_metadata resolved before analyze() Ã¢â‚¬â€ provider_chain doesn't change
...
# winner name resolved AFTER analyze() Ã¢â‚¬â€ active_provider_name updated by EnsembleProvider
...
```python
    ...
    # analysis_source = RULE (set by _build_fallback_analysis, always Ã¢â‚¬â€ I-13)
...
failed Ã¢â‚¬â€ regardless of which provider was configured.

---

#### 15c. doc.provider Ã¢â‚¬â€ winner name, not composite
...
| Provider call failed Ã¢â€ â€™ fallback | `"fallback"` | `RULE` | Ã¢ÂÅ’ never |
...
- Pre-Sprint-5C rows: `doc.provider` may be `"ensemble(openai,internal)"`.
  `effective_analysis_source` maps `startswith("ensemble(")` Ã¢â€ â€™ `INTERNAL` (conservative).
...
- The pipeline uses `provider.provider_name` for `doc.provider` Ã¢â‚¬â€ unchanged.
- `_resolve_analysis_source()` logic (provider-object-based, pre-analyze) Ã¢â‚¬â€ unchanged.
...
Every downstream consumer relies on `doc.analysis_source` Ã¢â‚¬â€ never on `doc.provider`.
...
2. analyze_pending Ã¢â€ â€™ AnalysisPipeline.run(doc)
   Pre-analyze:
     trace_metadata = _resolve_trace_metadata(ensemble)
       Ã¢â€ â€™ {"ensemble_chain": ["openai", "internal"]}  (provider_chain property)

3. await ensemble.analyze(title, text, context)
   EnsembleProvider iterates providers in order:
     Ã¢â€ â€™ tries openai.analyze()   Ã¢â€ Â succeeds
     Ã¢â€ â€™ (internal.analyze() never called)
   ensemble._active_provider_name = "openai"   Ã¢â€ Â winner recorded (I-23)

   Fallback scenario (openai fails):
     Ã¢â€ â€™ tries openai.analyze()   Ã¢â€ Â raises RuntimeError
     Ã¢â€ â€™ tries internal.analyze() Ã¢â€ Â succeeds
     ensemble._active_provider_name = "internal"
     analysis_source (post-5C) Ã¢â€ â€™ INTERNAL Ã¢Å“â€¦ (correct, internal ran)

4. Post-analyze (Sprint-5C, success path)
   provider_name   = _resolve_runtime_provider_name(ensemble)
                   = ensemble.active_provider_name Ã¢â€ â€™ "openai"
   analysis_source = _resolve_analysis_source("openai")  Ã¢â€ â€™ EXTERNAL_LLM (I-24)
...
8. Reload from DB (_from_model)
   ...
   doc.effective_analysis_source Ã¢â€ â€™ returns doc.analysis_source  Ã¢â€ â€™ EXTERNAL_LLM

9. export_training_data(doc)
   metadata["analysis_source"] = effective_analysis_source.value  Ã¢â€ â€™ "external_llm"
   Ã¢â€ â€™ teacher-eligible Ã¢Å“â€¦ (I-16, I-19 satisfied)

10. extract_signal_candidates(doc)
    signal.analysis_source = effective_analysis_source.value  Ã¢â€ â€™ "external_llm"

11. ResearchBriefBuilder._to_brief_document(doc)
    brief_doc.analysis_source = effective_analysis_source.value  Ã¢â€ â€™ "external_llm"
```
...
**Error-path scenario** (all ensemble providers fail Ã¢â€ â€™ RuntimeError re-raised):
```
3'. All providers fail Ã¢â€ â€™ pipeline except branch Ã¢â€ â€™ _build_fallback_analysis()
    analysis_source  = RULE   (set by fallback builder, always)
    provider_name    = "ensemble(openai,internal)"  (unchanged, composite Ã¢â‚¬â€ pre-5C legacy)
    doc.analysis_source = RULE after apply_to_document()
    Ã¢â€ â€™ teacher-ineligible Ã¢Å“â€¦ (RULE never teacher Ã¢â‚¬â€ I-19)
    Ã¢â€ â€™ effective_analysis_source returns RULE (analysis_source is set)
```
...
- These rows are NOT corpus-eligible even if `openai` had won Ã¢â‚¬â€ intentional tradeoff (I-26)
...
| # | Rule |
|---|---|
| I-1 | `content_hash` is auto-computed Ã¢â‚¬â€ never set manually |
...
| I-6 | `AnalysisResult` has no DB table Ã¢â‚¬â€ scores are denormalized |
| I-7 | LLM output always arrives as validated `LLMAnalysisOutput` Ã¢â‚¬â€ never raw dict |
| I-8 | `spam_probability > 0.7` Ã¢â€ â€™ `priority_score Ã¢â€°Â¤ 3` |
| I-9 | status transitions are one-way |
| I-10 | `is_analyzed` and `status=analyzed` are set together, atomically |
| I-11 | `AnalysisResult.confidence_score` is in-memory only Ã¢â‚¬â€ NOT written to DB. The DB column `credibility_score` is computed as `1.0 - spam_probability` inside `update_analysis()` |
| I-12 | A document with `analysis_result=None` MUST NOT have `status=ANALYZED` set. `update_analysis(doc_id, None)` is a contract violation Ã¢â‚¬â€ caller must check for None and mark FAILED |
| I-13 | Deterministic fallback analysis must remain conservative and must not bypass the shared signal thresholding path |
| I-14 | `InternalCompanionProvider` implements `BaseAnalysisProvider` exactly Ã¢â‚¬â€ zero pipeline changes required for companion introduction |
| I-15 | Companion model endpoint MUST be localhost or allowlisted internal address Ã¢â‚¬â€ no external inference calls |
...
| I-18 | `AnalysisSource` is set at result creation time Ã¢â‚¬â€ immutable after `apply_to_document()` |
| I-19 | Rule-only documents (`analysis_source=RULE`) NEVER serve as distillation teacher signal |
| I-20 | `InternalModelProvider.provider_name` is always `"internal"`, `recommended_priority` Ã¢â€°Â¤ 5, `actionable=False`, `sentiment_label=NEUTRAL` Ã¢â‚¬â€ these are hard invariants, not configurable |
| I-21 | `InternalCompanionProvider.provider_name` is always `"companion"` Ã¢â‚¬â€ distinct from `"internal"` (heuristic). factory.py routes `"internal"` Ã¢â€ â€™ `InternalModelProvider`, `"companion"` Ã¢â€ â€™ `InternalCompanionProvider` |
...
| I-24 | `_resolve_runtime_provider_name(provider)` resolves the winner name AFTER `analyze()` succeeds using duck-typed `active_provider_name`. `_resolve_analysis_source(winner_name)` then derives the tier. Neither is called in the error/fallback path Ã¢â‚¬â€ only `RULE` is valid when analysis failed. |
| I-25 | `doc.provider` stores the **winning** provider name (e.g. `"openai"`, `"internal"`) Ã¢â‚¬â€ never the composite ensemble string. `doc.metadata["ensemble_chain"]` records the full ordered list for traceability. |
...
| I-27 | `export_training_data()` MUST enforce teacher-eligibility at the function level when `teacher_only=True`. Uses `doc.analysis_source` directly (not `effective_analysis_source`) Ã¢â‚¬â€ legacy rows without an explicit field are excluded. Ã¢Å“â€¦ Implemented. |
| I-28 | The `evaluate` CLI command compares teacher-labeled scores against rule-baseline scores (no LLM calls). This is the Sprint-6 baseline only Ã¢â‚¬â€ it does NOT represent companion-model accuracy until a real companion inference endpoint is configured. |
...
| I-31 | Teacher-only filtering MUST use `doc.analysis_source` directly (strict mode, not `effective_analysis_source`) Ã¢â‚¬â€ never `provider`, `ensemble_chain`, source name, title, or URL. |
...
| I-41 | `promotion_record.json` is an audit artifact only Ã¢â‚¬â€ it does NOT change provider routing. Routing is controlled exclusively by env vars. |
...
| I-47 | `PromotionRecord` MUST embed `gates_summary: dict[str, bool]` Ã¢â‚¬â€ a snapshot of all 6 gate pass/fail results at record creation time. A promotion record without gate evidence is incomplete. |
...
| I-52 | Shadow run calls `InternalCompanionProvider.analyze()` directly and explicitly Ã¢â‚¬â€ independent of `APP_LLM_PROVIDER`. Shadow run is a separate, explicit audit call, never a routing override. |
...
| I-54 | Shadow run requires `companion_model_endpoint` to be configured. If absent, the command exits 0 with an informational message Ã¢â‚¬â€ not an error. |
...
| I-56 | Live shadow (inline `--shadow` flag in `analyze-pending`/`pipeline run`): Shadow provider runs concurrent to Primary inside `AnalysisPipeline.run()`. Both launched as `asyncio.create_task()`; Primary is awaited first. Shadow exception is caught non-blocking Ã¢â‚¬â€ `shadow_error` set, primary unaffected. |
| I-57 | Live shadow persistence: `update_analysis()` receives `metadata_updates=res.document.metadata` (after `apply_to_document()`) Ã¢â‚¬â€ NOT `res.trace_metadata`. This ensures `shadow_analysis` and `shadow_provider` written by `apply_to_document()` reach the DB `document_metadata` column. Enforced in both `run_rss_pipeline()` and `analyze-pending`. |
| I-58 | `DistillationReadinessReport` is a readiness assessment only. It MUST NOT trigger training, weight updates, or provider routing changes. `promotion_validation.is_promotable=True` is informational Ã¢â‚¬â€ the operator must still use `record-promotion` explicitly (I-36, I-39). |
...
| I-62 | `build_distillation_report()` is pure computation Ã¢â‚¬â€ no DB reads, no LLM calls, no network. All I/O is JSONL/JSON file reads via `load_jsonl()` and `json.loads()`. |
...
| I-65 | Post-training evaluation MUST use the same promotion gates G1Ã¢â‚¬â€œG6 as pre-promotion evaluation. `validate_promotion()` is the canonical gate Ã¢â‚¬â€ no Sprint-12 bypass is permitted. |
...
| I-68 | `record-promotion` remains the sole promotion gate. `TrainingJobRecord` and `PostTrainingEvaluationSpec` are audit artifacts only Ã¢â‚¬â€ they do not trigger or substitute promotion. |
| I-69 | Sprint-12 canonicalizes shadow JSONL schema: `shadow.py` MUST write `"deviations"` field (with `priority_delta`, `relevance_delta`, `impact_delta`) as canonical Ã¢â‚¬â€ matching `evaluation.py`. `"divergence"` remains as deprecated backward-compat alias. `compute_shadow_coverage()` continues to normalize both formats until old shadow files are migrated. |
| I-70 | `EvaluationComparisonReport` is a comparison artifact only Ã¢â‚¬â€ no routing change, no promotion trigger, no G1Ã¢â‚¬â€œG6 gate bypass. |
| I-71 | `compare_evaluation_reports(baseline_report, candidate_report)` takes `EvaluationReport` objects Ã¢â‚¬â€ it is pure computation. No DB reads, no LLM calls, no network. The CLI `compare-evaluations` handles file loading via `load_saved_evaluation_report()` before calling this function. (I-62 extends here.) |
| I-72 | When `regression_summary.has_regression=True` in the comparison report and `--comparison` is provided to `record-promotion`, a prominent WARNING is printed. Promotion is NOT automatically blocked Ã¢â‚¬â€ the operator must explicitly decide to proceed. `PromotionRecord.comparison_report_path` is set for the audit trail. Hard regression per-metric thresholds (R1Ã¢â‚¬â€œR6) are deferred; `has_regression` (any worsening) is the current operative flag. |
...
| I-76 | `UpgradeCycleReport.status` is derived exclusively from artifact presence (`Path.exists()`) Ã¢â‚¬â€ never auto-advanced by the platform. No platform code advances `status` without the operator supplying a new artifact path. |
...
| I-79 | Each `UpgradeCycleReport` represents one upgrade cycle attempt. Parallel or sequential cycles (e.g. v1Ã¢â€ â€™v2, v2Ã¢â€ â€™v3) produce separate files. A cycle report MUST NOT be overwritten in-place Ã¢â‚¬â€ re-runs produce new files (I-38 extends here). |
...
| I-93 | `ABCInferenceEnvelope` produced during a shadow-enabled analyze-pending run is written per-document to audit JSONL only Ã¢â‚¬â€ no DB writes, no routing changes. |
