ï»¿# Contracts and Core Data Models
...
| current_sprint | `PH5B_LOW_SIGNAL_CLUSTER_ANALYSIS (ready to close, Â§84)` |
| next_required_step | `PH5B_RESULTS_REVIEW_AND_PH5C_DEFINITION` |
| baseline | `1609 passed, ruff clean` |
| archive | `docs/contracts_archive.md` (closed Â§Â§38-Â§82) |

## Navigation

| Section | Content |
|---|---|
| [Core Contracts](#core-contracts) | Â§0-Â§15: Domain models, invariants, intelligence stack |
| [Immutable Invariants](#immutable-invariants) | Non-negotiable runtime rules |
| [Â§83 PH5A](#s83--ph5a_baseline_reliability_and_signal_trust) | Closed sprint contract |
| [Â§84 PH5B](#s84--sprint-ph5b_low_signal_cluster_analysis-d-92-2026-03-24) | Ready-to-close sprint contract |
| [Archive](contracts_archive.md) | Closed Â§Â§38-Â§82 (Phase 1-4) |
...
@dataclass
class FetchItem:
    url: str                        # required ÃÂ¢Ã¢âÂ¬Ã¢â¬Â canonical item URL
    external_id: str | None = None  # source-assigned ID (RSS guid, API id, ÃÂ¢Ã¢âÂ¬ÃÂ¦)
...
Rules:
- **No analysis** ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no scores, no sentiment, no priority, no tickers, no entity mentions
- **No persistence state** ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no `status`, `is_analyzed`, `is_duplicate`, `content_hash`, `id`
- **No source metadata** ÃÂ¢Ã¢âÂ¬Ã¢â¬Â `source_id`, `source_name`, `source_type` are injected by `normalize_fetch_item()`, never by the adapter
- As close to the source as possible ÃÂ¢Ã¢âÂ¬Ã¢â¬Â minimal transformation before `normalize_fetch_item()`
- `metadata` is a free-form bag for source-specific extras (image URL, author, feed tags, ÃÂ¢Ã¢âÂ¬ÃÂ¦)
...
Normalization is adapter-owned ÃÂ¢Ã¢âÂ¬Ã¢â¬Â it must NOT move into `persist_fetch_result()`, which is a
...
class FetchResult:
    source_id: str
    documents: list[CanonicalDocument]  # never None ÃÂ¢Ã¢âÂ¬Ã¢â¬Â empty list on failure
    fetched_at: datetime
    success: bool
...
Rules:
- adapter must never raise ÃÂ¢Ã¢âÂ¬Ã¢â¬Â catch all exceptions internally
...
- `content_hash` must not be set by adapter ÃÂ¢Ã¢âÂ¬Ã¢â¬Â auto-computed by `CanonicalDocument`
...
class CanonicalDocument(BaseModel):
    id: UUID                            # primary key ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never change after persist
    url: str                            # required ÃÂ¢Ã¢âÂ¬Ã¢â¬Â dedup key
    title: str                          # required
    ...
    content_hash: str | None = None     # auto-computed ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never set manually
    status: DocumentStatus              # lifecycle state ÃÂ¢Ã¢âÂ¬Ã¢â¬Â see below
...
Rules:
- `content_hash` is auto-computed from `url|title|raw_text` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never set manually
- `word_count` is a `@computed_field` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never stored in DB
...
class AnalysisResult(BaseModel):
    ...
    relevance_score: float              # [0.0, 1.0] ÃÂ¢Ã¢âÂ¬Ã¢â¬Â blended with keyword hits by apply_to_document()
    impact_score: float                 # [0.0, 1.0]
    novelty_score: float                # [0.0, 1.0]
    confidence_score: float             # [0.0, 1.0] ÃÂ¢Ã¢âÂ¬Ã¢â¬Â in-memory only, NOT persisted to DB
    ...
    explanation_short: str              # required ÃÂ¢Ã¢âÂ¬Ã¢â¬Â concise reasoning
    explanation_long: str               # required ÃÂ¢Ã¢âÂ¬Ã¢â¬Â full reasoning
...
Rules:
- Must be fully populated ÃÂ¢Ã¢âÂ¬Ã¢â¬Â all score fields are required (no optional scores)
- Must be schema-validated ÃÂ¢Ã¢âÂ¬Ã¢â¬Â all ranges enforced by Pydantic
...
- `spam_probability` IS stored on `AnalysisResult` for audit ÃÂ¢Ã¢âÂ¬Ã¢â¬Â but scoring functions
  (`compute_priority`, `is_alert_worthy`) receive it as an **explicit separate parameter**
- `recommended_priority` is set by `apply_to_document()` after `compute_priority()` runs ÃÂ¢Ã¢âÂ¬Ã¢â¬Â not by the LLM
- `AnalysisResult` is in-memory only ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no separate DB table
...
```
pending ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ persisted ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ analyzed
         ÃÂ¢Ã¢â¬Â ÃÅ failed
         ÃÂ¢Ã¢â¬Â ÃÅ duplicate
```

| Status | Meaning | Owner |
|---|---|---|
| `pending` | in-memory only ÃÂ¢Ã¢âÂ¬Ã¢â¬Â not yet saved to DB | `prepare_ingested_document()` in `document_ingest.py` |
...
| `failed` | non-recoverable error ÃÂ¢Ã¢âÂ¬Ã¢â¬Â kept for audit | `repo.update_status(FAILED)` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â ingest, `run_rss_pipeline()`, and `analyze_pending` CLI error handlers |
| `duplicate` | blocked at dedup gate ÃÂ¢Ã¢âÂ¬Ã¢â¬Â NOT saved | detected in-memory; `repo.mark_duplicate()` for retroactive marking |
...
Rules:
- transitions are one-way ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no rollback, no recycling
...
| Boundary | Rule |
|---|---|
| Ingestion ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ Storage | adapter returns `FetchResult`; only `persist_fetch_result()` persists |
| Storage ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ Analysis | `repo.get_pending_documents()` feeds the analysis queue ÃÂ¢Ã¢âÂ¬Ã¢â¬Â filters `status=PERSISTED` (not just flags) |
| Analysis ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ Storage | `apply_to_document()` then `repo.update_analysis()` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no other path |
| Analysis ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ Alerting | `is_alert_worthy()` is the only gate ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no direct score access |
| LLM calls | always via `BaseAnalysisProvider.analyze()` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never direct SDK calls |
| Config | always via `AppSettings` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never `os.environ` directly |
...
```
raw = (relevance ÃÆÃ¢â¬â 0.30) + (impact ÃÆÃ¢â¬â 0.30) + (novelty ÃÆÃ¢â¬â 0.20)
    + (actionable ÃÆÃ¢â¬â 0.15) + ((1 - spam) ÃÆÃ¢â¬â 0.05)

priority = round(raw ÃÆÃ¢â¬â 9) + 1          # maps [0.0, 1.0] ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ [1, 10]
...
Cap: if `spam_probability > 0.7` ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ `priority = min(priority, 3)` (applied after bonus)

Scale:
- 8ÃÂ¢Ã¢âÂ¬Ã¢â¬Å10: high urgency, actionable
- 6ÃÂ¢Ã¢âÂ¬Ã¢â¬Å7: notable, alert-worthy
- 4ÃÂ¢Ã¢âÂ¬Ã¢â¬Å5: background, low urgency
- 1ÃÂ¢Ã¢âÂ¬Ã¢â¬Å3: noise or spam
...
Rules:
- `run()` input is always `CanonicalDocument` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never a raw dict or ORM model
- `run()` output is always `PipelineResult` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never raises (errors surfaced in `result.error`)
...
Scoring is part of the pipeline result ÃÂ¢Ã¢âÂ¬Ã¢â¬Â not a separate side-effect.
...
3. title hash match (score 0.85 ÃÂ¢Ã¢âÂ¬Ã¢â¬Â catches same headline across sources)

Rules:
- conservative by default ÃÂ¢Ã¢âÂ¬Ã¢â¬Â prefer false negatives over false positives
- `is_duplicate()` never writes to DB ÃÂ¢Ã¢âÂ¬Ã¢â¬Â read-only
- dedup is enforced exclusively by `document_ingest.py` before `repo.save_document()`
- `filter_scored()` is used by `persist_fetch_result()` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â returns all docs with scores for auditing
...
Rules:
- every provider must return a fully validated `LLMAnalysisOutput` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never a raw dict (I-7)
- `analyze()` never receives a `CanonicalDocument` directly ÃÂ¢Ã¢âÂ¬Ã¢â¬Â caller extracts `title` + `text`
...
- structured output enforcement is provider-specific (OpenAI: `response_format`, Anthropic: tool-use,
  Gemini: `response_schema`) ÃÂ¢Ã¢âÂ¬Ã¢â¬Â but the output contract is identical
...
- never trust provider output blindly ÃÂ¢Ã¢âÂ¬Ã¢â¬Â schema validation is mandatory

---

### 11. Sprint 4 ÃÂ¢Ã¢âÂ¬Ã¢â¬Â Research & Signal Contracts

These contracts define the Sprint 4 output layer. All three types are **in-memory only** ÃÂ¢Ã¢âÂ¬Ã¢â¬Â
...
Rules:
- Source: `monitor/watchlists.yml` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â loaded via `WatchlistEntry` + `load_watchlist()`
...
- `WatchlistRegistry` is read-only after construction ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no mutations during runtime
- `load_watchlist()` returns `[]` (not an error) if the file does not exist
- `find_by_text()` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â Sprint 4B planned, not yet implemented; use `filter_documents()` instead
...
class BriefDocument(BaseModel):
    document_id: str          # str(CanonicalDocument.id) ÃÂ¢Ã¢âÂ¬Ã¢â¬Â traceability
    title: str
    url: str
...
Rules:
- Input: `list[CanonicalDocument]` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â only `is_analyzed=True` docs are used
- `ResearchBriefBuilder.build()` never raises ÃÂ¢Ã¢âÂ¬Ã¢â¬Â returns empty brief on empty/unanalyzed input
- `_ACTIONABLE_PRIORITY_THRESHOLD = 8` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â must stay in sync with `ThresholdEngine.min_priority`
...
- `ResearchBrief` is in-memory only ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no DB table, no persistence
...
class SignalCandidate(BaseModel):
    ...
    signal_id: str              # f"sig_{document_id}" ÃÂ¢Ã¢âÂ¬Ã¢â¬Â deterministic
    document_id: str            # str(CanonicalDocument.id) ÃÂ¢Ã¢âÂ¬Ã¢â¬Â traceability

    target_asset: str           # primary asset ("BTC", "ETH", "General Market")
    direction_hint: str         # "bullish" | "bearish" | "neutral"
                                # NEVER "buy" / "sell" / "hold" ÃÂ¢Ã¢âÂ¬Ã¢â¬Â not an execution instruction
    confidence: float           # proxy: doc.relevance_score ÃÂ¢Ã¢âÂ¬Ã¢â¬Â [0.0, 1.0]
    supporting_evidence: str    # doc.summary or doc.title
    contradicting_evidence: str # static note ÃÂ¢Ã¢âÂ¬Ã¢â¬Â not extracted in primary scan
    risk_notes: str             # spam_prob + market_scope metadata
    source_quality: float       # doc.credibility_score ÃÂ¢Ã¢âÂ¬Ã¢â¬Â [0.0, 1.0]
    recommended_next_step: str  # always ends with "ÃÂ¢Ã¢âÂ¬Ã¢â¬Â human decision required."
...
Rules:
- `priority >= 8` is a hard constraint ÃÂ¢Ã¢âÂ¬Ã¢â¬Â Pydantic `Field(ge=8)` enforced at construction
- `direction_hint` is research language, NOT trading instruction ÃÂ¢Ã¢âÂ¬Ã¢â¬Â "bullish"/"bearish"/"neutral"
- `signal_id` is deterministic: `f"sig_{document_id}"` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â idempotent for same document
- `watchlist_boosts`: `{"BTC": 1}` raises effective priority by 1 for watchlist assets;
  capped at 10; never raises above 10
- `confidence_score` from `AnalysisResult` is NOT persisted to DB ÃÂ¢Ã¢âÂ¬Ã¢â¬Â `relevance_score` is
  used as the confidence proxy (available in DB)
- `SignalCandidate` is in-memory only ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no DB table, no persistence
- `extract_signal_candidates()` never raises ÃÂ¢Ã¢âÂ¬Ã¢â¬Â returns `[]` if no candidates qualify
...
| No DB writes | `ResearchBrief` and `SignalCandidate` are always in-memory ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never persisted |
| No LLM calls | Research layer is pure computation ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no provider calls, no external I/O |
| Watchlist source | Always from `monitor/watchlists.yml` via `WatchlistRegistry.from_monitor_dir()` |
| CLI entry point | `research` Typer subgroup ÃÂ¢Ã¢âÂ¬Ã¢â¬Â `watchlists`, `brief`, `signals` commands |
...
**Status: ÃÂ¢ÃâÃ¢â¬Â¦ Implemented** (`app/core/settings.py`)
...
**Status: ÃÂ¢ÃâÃ¢â¬Â¦ Implemented** (`app/analysis/factory.py`)
...
`EnsembleProvider` (`app/analysis/ensemble/provider.py`) is not a factory target ÃÂ¢Ã¢âÂ¬Ã¢â¬Â it wraps
multiple providers directly. Its `provider_name` is a compound string like
`"ensemble(openai,internal)"` (see ÃâÃÂ§13e on EnsembleProvider and analysis_source).

---

#### 13c. AnalysisSource Enum

**Status: ÃÂ¢ÃâÃ¢â¬Â¦ Implemented** (`app/core/enums.py`, `app/core/domain/document.py`)

```python
# app/core/enums.py
class AnalysisSource(StrEnum):
    RULE = "rule"                  # Tier 1 ÃÂ¢Ã¢âÂ¬Ã¢â¬Â fallback / rule-based heuristics
    INTERNAL = "internal"          # Tier 2 ÃÂ¢Ã¢âÂ¬Ã¢â¬Â InternalModelProvider or InternalCompanionProvider
    EXTERNAL_LLM = "external_llm"  # Tier 3 ÃÂ¢Ã¢âÂ¬Ã¢â¬Â OpenAI / Anthropic / Gemini
```
...
```python
# app/core/domain/document.py ÃÂ¢Ã¢âÂ¬Ã¢â¬Â compatibility accessor
...
- Distillation corpus selects ONLY `analysis_source=EXTERNAL_LLM` documents (ÃâÃÂ§14e)

---

#### 13d. Companion Model Output Scope

**Status: ÃÂ¢ÃâÃ¢â¬Â¦ Implemented** (`app/analysis/providers/companion.py`)
...
field (ÃâÃÂ§14c). The internal reasoning trace is not part of the training corpus output format.
...
1. `_resolve_analysis_source()` in `app/analysis/pipeline.py` maps any `provider_name` that
   starts with `"ensemble("` to `INTERNAL`. This is the primary guard ÃÂ¢Ã¢âÂ¬Ã¢â¬Â it sets
...
|--------------------------|----------------|--------------------------|----------------------------------|
| openai won | `"ensemble(openai,internal)"` | `"internal"` ÃÂ¢ÃÂ¡ÃÂ ÃÂ¯ÃÂ¸ÃÂ (conservative) | `"external_llm"` ÃÂ¢ÃâÃ¢â¬Â¦ |
| internal won | `"ensemble(openai,internal)"` | `"internal"` ÃÂ¢ÃâÃ¢â¬Â¦ (correct) | `"internal"` ÃÂ¢ÃâÃ¢â¬Â¦ |
...
```json
{
  "messages": [
    ...
    {"role": "assistant", "content": "<JSON target scores ÃÂ¢Ã¢âÂ¬Ã¢â¬Â sorted keys>"}
  ],
  "metadata": {
    "document_id":     "<uuid ÃÂ¢Ã¢âÂ¬Ã¢â¬Â str>",
...
|-------|------|----------|-------|
| `affected_assets` | list[str] | ÃÂ¢ÃâÃ¢â¬Â¦ | deduplicated from `doc.tickers + doc.crypto_assets` |
| `impact_score` | float | ÃÂ¢ÃâÃ¢â¬Â¦ | 0.0 .. 1.0 |
| `market_scope` | str | ÃÂ¢ÃâÃ¢â¬Â¦ | e.g. `"crypto"` / `"etf"` / `"unknown"` |
| `novelty_score` | float | ÃÂ¢ÃâÃ¢â¬Â¦ | 0.0 .. 1.0 |
| `priority_score` | int | ÃÂ¢ÃâÃ¢â¬Â¦ | 1 .. 10 |
| `relevance_score` | float | ÃÂ¢ÃâÃ¢â¬Â¦ | 0.0 .. 1.0 |
| `sentiment_label` | str | ÃÂ¢ÃâÃ¢â¬Â¦ | `"bullish"` / `"bearish"` / `"neutral"` |
| `sentiment_score` | float | ÃÂ¢ÃâÃ¢â¬Â¦ | -1.0 .. 1.0 |
| `spam_probability` | float | ÃÂ¢ÃâÃ¢â¬Â¦ | 0.0 .. 1.0 |
| `summary` | str | ÃÂ¢ÃâÃ¢â¬Â¦ | `doc.summary` or `""` |
| `tags` | list[str] | ÃÂ¢ÃâÃ¢â¬Â¦ | `doc.ai_tags` |

All fields are always present (no optional fields in the assistant target).

---

#### 14c. `co_thought` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â Final Decision: REMOVED
...
1. **Contamination risk**: Rule-based analysis sets `explanation_short = "Rule-based fallback
   analysis. ..."` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â a heuristic label, not reasoning. Including it as chain-of-thought
...
3. **Schema coupling**: `co_thought` would couple the export to `doc.metadata["explanation_short"]`
   ÃÂ¢Ã¢âÂ¬Ã¢â¬Â an implementation detail, not a stable contract field.
...
|-------|---------|----------------------|-----------------|
| `"external_llm"` | External LLM (OpenAI, Anthropic, Gemini) | `"openai"`, `"anthropic"`, `"gemini"`, etc. | ÃÂ¢ÃâÃ¢â¬Â¦ yes |
| `"internal"` | Tier 2 analysis (heuristic or companion HTTP) | `"internal"`, `"companion"` | ÃÂ¢ÃÂ¡ÃÂ ÃÂ¯ÃÂ¸ÃÂ evaluation only |
| `"rule"` | Rule-based / fallback analysis | `None`, `"fallback"`, `"rule"` | ÃÂ¢ÃÂÃâ no (I-19) |
...
# ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ returns doc.analysis_source if explicitly set (Sprint 5B field)
# ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ falls back to derivation from doc.provider (legacy path for pre-Sprint-5B rows)
```

The export reads `doc.effective_analysis_source.value` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never derives analysis_source inline.
...
- `doc.provider` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â may be `"openai"` (teacher), `"internal"` (not teacher), or a legacy
  composite `"ensemble(openai,internal)"` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â ambiguous without `analysis_source`
- `doc.metadata["ensemble_chain"]` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â audit trail only, not a classification signal
...
#### 14f. provider vs analysis_source ÃÂ¢Ã¢âÂ¬Ã¢â¬Â Contract Separation

These are two distinct concepts that must never be conflated:

| Concept | Field | Type | Persistence | Purpose |
|---------|-------|------|-------------|---------|
| `provider` | `doc.provider` | `str \| None` | DB column | Technical engine name. Pre-5C: `"openai"`, `"internal"`, `"ensemble(openai,internal)"`, `"fallback"`. Post-5C: always the **winner name** ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never a composite string. |
| `analysis_source` | `doc.analysis_source` | `AnalysisSource` enum | DB column (migration 0006) | Semantic tier: `RULE` / `INTERNAL` / `EXTERNAL_LLM` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â stable, use this for filtering. |
...
**Rules:**
- `provider` is a technical string ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never use it directly for corpus filtering or tier decisions
- `analysis_source` is the stable semantic value ÃÂ¢Ã¢âÂ¬Ã¢â¬Â always use this for filtering and guardrails
- `provider` semantics changed in Sprint-5C: composite `"ensemble(...)"` strings are legacy only
- `analysis_source` must NEVER be set manually ÃÂ¢Ã¢âÂ¬Ã¢â¬Â always set by pipeline at result creation time
- Downstream code (ResearchBrief, SignalCandidate, alerts) consumes analysis results via the same
  `CanonicalDocument` contract regardless of which tier produced them ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no branching on `provider`
...
- `ResearchBrief.key_documents` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â ÃÂ¢ÃâÃ¢â¬Â¦ included
- `ResearchBrief.top_actionable_signals` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â ÃÂ¢ÃâÃ¢â¬Â¦ if `priority >= 8`
- `SignalCandidate` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â ÃÂ¢ÃâÃ¢â¬Â¦ if `priority >= 8` (companion can reach 8 with strong output)
- Alert gating ÃÂ¢Ã¢âÂ¬Ã¢â¬Â ÃÂ¢ÃâÃ¢â¬Â¦ same `ThresholdEngine.is_alert_worthy()` path
...
### 15. Sprint-5C ÃÂ¢Ã¢âÂ¬Ã¢â¬Â Winner-Traceability Contract
...
`doc.analysis_source = INTERNAL` and `doc.provider = "ensemble(openai,internal)"` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â
...
def _resolve_runtime_provider_name(provider: BaseAnalysisProvider | None) -> str | None:
    """Return the actual winner name after analyze() ÃÂ¢Ã¢âÂ¬Ã¢â¬Â for EnsembleProvider via duck typing."""
    if provider is None:
        return None
...
pipelines ÃÂ¢Ã¢âÂ¬Ã¢â¬Â `_resolve_runtime_provider_name` returns the winner name instead. The composite
guard remains in `CanonicalDocument.effective_analysis_source` for legacy DB rows only (ÃâÃÂ§15e).

**Pipeline call site** (success path):

```python
# trace_metadata resolved before analyze() ÃÂ¢Ã¢âÂ¬Ã¢â¬Â provider_chain doesn't change
...
# winner name resolved AFTER analyze() ÃÂ¢Ã¢âÂ¬Ã¢â¬Â active_provider_name updated by EnsembleProvider
...
```python
    ...
    # analysis_source = RULE (set by _build_fallback_analysis, always ÃÂ¢Ã¢âÂ¬Ã¢â¬Â I-13)
...
failed ÃÂ¢Ã¢âÂ¬Ã¢â¬Â regardless of which provider was configured.

---

#### 15c. doc.provider ÃÂ¢Ã¢âÂ¬Ã¢â¬Â winner name, not composite
...
|---|---|---|---|
| No provider configured | `"fallback"` | `RULE` | ÃÂ¢ÃÂÃâ never |
| Provider call failed ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ fallback | `"fallback"` | `RULE` | ÃÂ¢ÃÂÃâ never |
| InternalModelProvider ran | `"internal"` | `INTERNAL` | ÃÂ¢ÃÂ¡ÃÂ ÃÂ¯ÃÂ¸ÃÂ eval only |
| InternalCompanionProvider ran | `"companion"` | `INTERNAL` | ÃÂ¢ÃÂ¡ÃÂ ÃÂ¯ÃÂ¸ÃÂ eval only |
| OpenAI ran (direct) | `"openai"` | `EXTERNAL_LLM` | ÃÂ¢ÃâÃ¢â¬Â¦ teacher |
| Ensemble: openai won | `"openai"` (from `ensemble.model`) | `EXTERNAL_LLM` | ÃÂ¢ÃâÃ¢â¬Â¦ teacher |
| Ensemble: internal fallback | `"internal"` (from `ensemble.model`) | `INTERNAL` | ÃÂ¢ÃÂ¡ÃÂ ÃÂ¯ÃÂ¸ÃÂ eval only |
| Ensemble: companion fallback | `"companion"` (from `ensemble.model`) | `INTERNAL` | ÃÂ¢ÃÂ¡ÃÂ ÃÂ¯ÃÂ¸ÃÂ eval only |
...
- Pre-Sprint-5C rows: `doc.provider` may be `"ensemble(openai,internal)"`.
  `effective_analysis_source` maps `startswith("ensemble(")` ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ `INTERNAL` (conservative).
...
- The pipeline uses `provider.provider_name` for `doc.provider` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â unchanged.
- `_resolve_analysis_source()` logic (provider-object-based, pre-analyze) ÃÂ¢Ã¢âÂ¬Ã¢â¬Â unchanged.
...
Every downstream consumer relies on `doc.analysis_source` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never on `doc.provider`.
...
2. analyze_pending ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ AnalysisPipeline.run(doc)
   Pre-analyze:
     trace_metadata = _resolve_trace_metadata(ensemble)
       ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ {"ensemble_chain": ["openai", "internal"]}  (provider_chain property)

3. await ensemble.analyze(title, text, context)
   EnsembleProvider iterates providers in order:
     ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ tries openai.analyze()   ÃÂ¢Ã¢â¬Â ÃÂ succeeds
     ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ (internal.analyze() never called)
   ensemble._active_provider_name = "openai"   ÃÂ¢Ã¢â¬Â ÃÂ winner recorded (I-23)

   Fallback scenario (openai fails):
     ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ tries openai.analyze()   ÃÂ¢Ã¢â¬Â ÃÂ raises RuntimeError
     ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ tries internal.analyze() ÃÂ¢Ã¢â¬Â ÃÂ succeeds
     ensemble._active_provider_name = "internal"
     analysis_source (post-5C) ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ INTERNAL ÃÂ¢ÃâÃ¢â¬Â¦ (correct, internal ran)

4. Post-analyze (Sprint-5C, success path)
   provider_name   = _resolve_runtime_provider_name(ensemble)
                   = ensemble.active_provider_name ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ "openai"
   analysis_source = _resolve_analysis_source("openai")  ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ EXTERNAL_LLM (I-24)
...
8. Reload from DB (_from_model)
   ...
   doc.effective_analysis_source ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ returns doc.analysis_source  ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ EXTERNAL_LLM

9. export_training_data(doc)
   metadata["analysis_source"] = effective_analysis_source.value  ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ "external_llm"
   ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ teacher-eligible ÃÂ¢ÃâÃ¢â¬Â¦ (I-16, I-19 satisfied)

10. extract_signal_candidates(doc)
    signal.analysis_source = effective_analysis_source.value  ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ "external_llm"

11. ResearchBriefBuilder._to_brief_document(doc)
    brief_doc.analysis_source = effective_analysis_source.value  ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ "external_llm"
```

**Consistency invariant**: All consumers in steps 9ÃÂ¢Ã¢âÂ¬Ã¢â¬Å11 read `doc.effective_analysis_source`.
...
**Error-path scenario** (all ensemble providers fail ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ RuntimeError re-raised):
```
3'. All providers fail ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ pipeline except branch ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ _build_fallback_analysis()
    analysis_source  = RULE   (set by fallback builder, always)
    provider_name    = "ensemble(openai,internal)"  (unchanged, composite ÃÂ¢Ã¢âÂ¬Ã¢â¬Â pre-5C legacy)
    doc.analysis_source = RULE after apply_to_document()
    ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ teacher-ineligible ÃÂ¢ÃâÃ¢â¬Â¦ (RULE never teacher ÃÂ¢Ã¢âÂ¬Ã¢â¬Â I-19)
    ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ effective_analysis_source returns RULE (analysis_source is set)
```
...
- These rows are NOT corpus-eligible even if `openai` had won ÃÂ¢Ã¢âÂ¬Ã¢â¬Â intentional tradeoff (I-26)
...
| # | Rule |
|---|---|
| I-1 | `content_hash` is auto-computed ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never set manually |
...
| I-6 | `AnalysisResult` has no DB table ÃÂ¢Ã¢âÂ¬Ã¢â¬Â scores are denormalized |
| I-7 | LLM output always arrives as validated `LLMAnalysisOutput` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never raw dict |
| I-8 | `spam_probability > 0.7` ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ `priority_score ÃÂ¢Ã¢â¬Â°ÃÂ¤ 3` |
| I-9 | status transitions are one-way |
| I-10 | `is_analyzed` and `status=analyzed` are set together, atomically |
| I-11 | `AnalysisResult.confidence_score` is in-memory only ÃÂ¢Ã¢âÂ¬Ã¢â¬Â NOT written to DB. The DB column `credibility_score` is computed as `1.0 - spam_probability` inside `update_analysis()` |
| I-12 | A document with `analysis_result=None` MUST NOT have `status=ANALYZED` set. `update_analysis(doc_id, None)` is a contract violation ÃÂ¢Ã¢âÂ¬Ã¢â¬Â caller must check for None and mark FAILED |
| I-13 | Deterministic fallback analysis must remain conservative and must not bypass the shared signal thresholding path |
| I-14 | `InternalCompanionProvider` implements `BaseAnalysisProvider` exactly ÃÂ¢Ã¢âÂ¬Ã¢â¬Â zero pipeline changes required for companion introduction |
| I-15 | Companion model endpoint MUST be localhost or allowlisted internal address ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no external inference calls |
| I-16 | Distillation corpus uses only `analysis_source=EXTERNAL_LLM` documents as teacher signal |
| I-17 | Companion model `impact_score` cap: ÃÂ¢Ã¢â¬Â°ÃÂ¤ 0.8 (conservative, not overconfident) |
| I-18 | `AnalysisSource` is set at result creation time ÃÂ¢Ã¢âÂ¬Ã¢â¬Â immutable after `apply_to_document()` |
| I-19 | Rule-only documents (`analysis_source=RULE`) NEVER serve as distillation teacher signal |
| I-20 | `InternalModelProvider.provider_name` is always `"internal"`, `recommended_priority` ÃÂ¢Ã¢â¬Â°ÃÂ¤ 5, `actionable=False`, `sentiment_label=NEUTRAL` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â these are hard invariants, not configurable |
| I-21 | `InternalCompanionProvider.provider_name` is always `"companion"` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â distinct from `"internal"` (heuristic). factory.py routes `"internal"` ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ `InternalModelProvider`, `"companion"` ÃÂ¢Ã¢â¬Â Ã¢â¬â¢ `InternalCompanionProvider` |
...
| I-24 | `_resolve_runtime_provider_name(provider)` resolves the winner name AFTER `analyze()` succeeds using duck-typed `active_provider_name`. `_resolve_analysis_source(winner_name)` then derives the tier. Neither is called in the error/fallback path ÃÂ¢Ã¢âÂ¬Ã¢â¬Â only `RULE` is valid when analysis failed. |
| I-25 | `doc.provider` stores the **winning** provider name (e.g. `"openai"`, `"internal"`) ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never the composite ensemble string. `doc.metadata["ensemble_chain"]` records the full ordered list for traceability. |
...
| I-27 | `export_training_data()` MUST enforce teacher-eligibility at the function level when `teacher_only=True`. Uses `doc.analysis_source` directly (not `effective_analysis_source`) ÃÂ¢Ã¢âÂ¬Ã¢â¬Â legacy rows without an explicit field are excluded. ÃÂ¢ÃâÃ¢â¬Â¦ Implemented. |
| I-28 | The `evaluate` CLI command compares teacher-labeled scores against rule-baseline scores (no LLM calls). This is the Sprint-6 baseline only ÃÂ¢Ã¢âÂ¬Ã¢â¬Â it does NOT represent companion-model accuracy until a real companion inference endpoint is configured. |
...
| I-31 | Teacher-only filtering MUST use `doc.analysis_source` directly (strict mode, not `effective_analysis_source`) ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never `provider`, `ensemble_chain`, source name, title, or URL. |
...
| I-41 | `promotion_record.json` is an audit artifact only ÃÂ¢Ã¢âÂ¬Ã¢â¬Â it does NOT change provider routing. Routing is controlled exclusively by env vars. |
...
| I-45 | `record-promotion` and `save_promotion_record()` require the evaluation report to exist and pass all 6 quantitative gates (G1ÃÂ¢Ã¢âÂ¬Ã¢â¬ÅG6). Non-passing reports block record creation. |
...
| I-47 | `PromotionRecord` MUST embed `gates_summary: dict[str, bool]` ÃÂ¢Ã¢âÂ¬Ã¢â¬Â a snapshot of all 6 gate pass/fail results at record creation time. A promotion record without gate evidence is incomplete. |
...
| I-52 | Shadow run calls `InternalCompanionProvider.analyze()` directly and explicitly ÃÂ¢Ã¢âÂ¬Ã¢â¬Â independent of `APP_LLM_PROVIDER`. Shadow run is a separate, explicit audit call, never a routing override. |
...
| I-54 | Shadow run requires `companion_model_endpoint` to be configured. If absent, the command exits 0 with an informational message ÃÂ¢Ã¢âÂ¬Ã¢â¬Â not an error. |
...
| I-56 | Live shadow (inline `--shadow` flag in `analyze-pending`/`pipeline run`): Shadow provider runs concurrent to Primary inside `AnalysisPipeline.run()`. Both launched as `asyncio.create_task()`; Primary is awaited first. Shadow exception is caught non-blocking ÃÂ¢Ã¢âÂ¬Ã¢â¬Â `shadow_error` set, primary unaffected. |
| I-57 | Live shadow persistence: `update_analysis()` receives `metadata_updates=res.document.metadata` (after `apply_to_document()`) ÃÂ¢Ã¢âÂ¬Ã¢â¬Â NOT `res.trace_metadata`. This ensures `shadow_analysis` and `shadow_provider` written by `apply_to_document()` reach the DB `document_metadata` column. Enforced in both `run_rss_pipeline()` and `analyze-pending`. |
| I-58 | `DistillationReadinessReport` is a readiness assessment only. It MUST NOT trigger training, weight updates, or provider routing changes. `promotion_validation.is_promotable=True` is informational ÃÂ¢Ã¢âÂ¬Ã¢â¬Â the operator must still use `record-promotion` explicitly (I-36, I-39). |
...
| I-62 | `build_distillation_report()` is pure computation ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no DB reads, no LLM calls, no network. All I/O is JSONL/JSON file reads via `load_jsonl()` and `json.loads()`. |
...
| I-65 | Post-training evaluation MUST use the same promotion gates G1ÃÂ¢Ã¢âÂ¬Ã¢â¬ÅG6 as pre-promotion evaluation. `validate_promotion()` is the canonical gate ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no Sprint-12 bypass is permitted. |
...
| I-68 | `record-promotion` remains the sole promotion gate. `TrainingJobRecord` and `PostTrainingEvaluationSpec` are audit artifacts only ÃÂ¢Ã¢âÂ¬Ã¢â¬Â they do not trigger or substitute promotion. |
| I-69 | Sprint-12 canonicalizes shadow JSONL schema: `shadow.py` MUST write `"deviations"` field (with `priority_delta`, `relevance_delta`, `impact_delta`) as canonical ÃÂ¢Ã¢âÂ¬Ã¢â¬Â matching `evaluation.py`. `"divergence"` remains as deprecated backward-compat alias. `compute_shadow_coverage()` continues to normalize both formats until old shadow files are migrated. |
| I-70 | `EvaluationComparisonReport` is a comparison artifact only ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no routing change, no promotion trigger, no G1ÃÂ¢Ã¢âÂ¬Ã¢â¬ÅG6 gate bypass. |
| I-71 | `compare_evaluation_reports(baseline_report, candidate_report)` takes `EvaluationReport` objects ÃÂ¢Ã¢âÂ¬Ã¢â¬Â it is pure computation. No DB reads, no LLM calls, no network. The CLI `compare-evaluations` handles file loading via `load_saved_evaluation_report()` before calling this function. (I-62 extends here.) |
| I-72 | When `regression_summary.has_regression=True` in the comparison report and `--comparison` is provided to `record-promotion`, a prominent WARNING is printed. Promotion is NOT automatically blocked ÃÂ¢Ã¢âÂ¬Ã¢â¬Â the operator must explicitly decide to proceed. `PromotionRecord.comparison_report_path` is set for the audit trail. Hard regression per-metric thresholds (R1ÃÂ¢Ã¢âÂ¬Ã¢â¬ÅR6) are deferred; `has_regression` (any worsening) is the current operative flag. |
...
| I-76 | `UpgradeCycleReport.status` is derived exclusively from artifact presence (`Path.exists()`) ÃÂ¢Ã¢âÂ¬Ã¢â¬Â never auto-advanced by the platform. No platform code advances `status` without the operator supplying a new artifact path. |
...
| I-79 | Each `UpgradeCycleReport` represents one upgrade cycle attempt. Parallel or sequential cycles (e.g. v1ÃÂ¢Ã¢â¬Â Ã¢â¬â¢v2, v2ÃÂ¢Ã¢â¬Â Ã¢â¬â¢v3) produce separate files. A cycle report MUST NOT be overwritten in-place ÃÂ¢Ã¢âÂ¬Ã¢â¬Â re-runs produce new files (I-38 extends here). |
...
| I-93 | `ABCInferenceEnvelope` produced during a shadow-enabled analyze-pending run is written per-document to audit JSONL only ÃÂ¢Ã¢âÂ¬Ã¢â¬Â no DB writes, no routing changes. |
...
## ÃÂ§83 Ã¢â¬â PH5A_BASELINE_RELIABILITY_AND_SIGNAL_TRUST
...
Diagnostic-only sprint Ã¢â¬â no code changes to production paths. Measure, record, freeze.

### Scope

Compute and record the following baseline metrics from the existing `69`-document paired set:

1. **Fallback rate** Ã¢â¬â fraction of documents taking the rule-only (fallback) path vs LLM path
2. **LLM error rate** Ã¢â¬â fraction of LLM calls that resulted in error/fallback
3. **Provider distribution** Ã¢â¬â which providers delivered results and in what proportion
4. **Priority distribution** Ã¢â¬â histogram of final `tier3_priority` scores (0Ã¢â¬â10)
5. **Actionable rate** Ã¢â¬â fraction of documents with `actionable=True`
6. **Keyword coverage** Ã¢â¬â fraction of documents with at least one keyword match
7. **Tag fill rate** Ã¢â¬â fraction of documents with at least one tag

### Deliverables

- `artifacts/ph5a_reliability_baseline.json` Ã¢â¬â machine-readable metrics
- `artifacts/ph5a_operator_summary.md` Ã¢â¬â human-readable summary
...
ÃÂ§83 status: **closed (D-90, 2026-03-24)**

---

## ÃÂ§84 Ã¢â¬â Sprint: PH5B_LOW_SIGNAL_CLUSTER_ANALYSIS (D-92, 2026-03-24)
...
- Fallback rate: 0.0% Ã¢â¬â pipeline parse errors resolved
- LLM error proxy rate: **27.5%** (19/69) Ã¢â¬â main gap
- Keyword coverage: 62.3% Ã¢â¬â improvement possible
- Tag fill rate: 100% Ã¢â¬â Phase 4 complete
- Actionable rate (Tier3): 0.0% Ã¢â¬â I-13 invariant confirmed
...
Â§84 status: **ready to close (D-94, 2026-03-24)**

---

## §85 — Sprint: PH5C_FILTER_BEFORE_LLM_BASELINE (D-95, 2026-03-24)

### Objective

Implement a pre-LLM stub/empty-content filter that detects and skips documents with no
...
§85 status: **active (D-95, 2026-03-24)**
