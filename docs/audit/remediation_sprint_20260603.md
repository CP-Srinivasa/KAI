...
| 4 | Bleed-/Loss-Circuit-Breaker | **ERLEDIGT (Detektion #134 + Enforcement #135)** | Detektion (#134): `risk_open`/`data_unknown` + WARN, read-only. **Enforcement (#135, `app/risk/promotion_gate.py` + CLI `trading promotion-check`)**: fail-closed Promotion-/Re-Enable-Stop — blockt risiko-erhöhende EntryMode-Hochstufung bei risk_open/data_unknown/stale/bleed/missing-artifact → `manual_review_required` (exit 1). De-Risking/Exits nie gegated. 12 Tests, kein auto_close, ändert entry_mode nicht. Live verifiziert: disabled→paper blockt (DOT-Bleed). Offen: optionale Verdrahtung in automatischen Promotion-Pfad / edge-gate-Downgrade. |
...
- ~~**P0** Enforcing-Hälfte Bleed-Breaker (#4)~~ → **ERLEDIGT in PR #135** (promotion_gate + `trading promotion-check`, fail-closed). Folge: optionale Auto-Promotion-Pfad-Verdrahtung + edge-gate-Downgrade.
- **P0** DOT/USDT-Operator-Entscheidung: blutende Position (−7.5 %) sauber schließen ODER als `manual_review`/frozen markieren — **nicht** durch neuen Entry-Code „reparieren". Der Promotion-Gate verhindert nur Re-Enable, nicht den weiteren Drawdown des Bestands.
