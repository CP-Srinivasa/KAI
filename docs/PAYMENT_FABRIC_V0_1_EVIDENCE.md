...
1. LIVE-Fenster nach Runbook: SHADOW-Preview (Decode/Quote/Policy) mit der Operator-Invoice → genau ein Send 1.000 sat, HOTP, kleines Fee-Limit → Journal `SETTLED` mit `proof_hash`/`fee_actual` → Reconciler bestätigt → Flags zurück → Ergebnis hier als VERIFIED nachtragen.
