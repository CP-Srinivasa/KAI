...
- **LIVE-Send am Gerät (2026-09-04 07:58:57–07:59:00 UTC) — VERIFIED:** Fenster `APP_PAYMENT_MODE=live`, `APP_LN_PAY_ENABLED=true`, Allowlist = `sha256(payee)`, Limits 1.000/1.000 sat, Fee-Max 5 sat, HOTP ab 1 sat. Intent `pi_f5648a5eb9854ae1` (Idempotency-Key `live-window-20260904-04`, Ziel: Operator-Invoice aus Wallet of Satoshi, 1.000 sat): `intent_created` → `policy_decided` (`REQUIRES_APPROVAL`) → `approval_granted` (HOTP vom YubiKey, Zähler 0) → `submitted` (1.000 sat, Versuch 1, `rail_dedup_key`) → `rail_responded` (`observed_status=SUCCEEDED`, `evidence_source=rail_response`) → `settled` (`amount_settled=1000`, `fee_actual=4`, `proof_hash=938d743e…`). Zweiter `execute` mit demselben Code → `replayed=true, already executed`, **kein zweiter Send**. Node-Lookup über `readonly.macaroon`: `SUCCEEDED`, `value_sat=1000`, `fee_sat=4`. Reconciler-Lauf 08:00:13 UTC: `ok`, `orphans=0` (Node-Zahlung dem Intent zugeordnet). `/health/payment`: `last_settlement 07:59:00Z / 1000`, `fees=4`, `settlement_latency_p50=2,9 s`, `policy_reject_count=1` (die initiale Allowlist-DENY). Gegenseite bestätigt: Operator meldet Eingang der 1.000 sat in Wallet of Satoshi (2026-09-04). Rückbau unmittelbar danach: `APP_PAYMENT_MODE=shadow`, `APP_LN_PAY_ENABLED=false`, Allowlist leer, `live_gate.pay_enabled=false` bestätigt.
- **Rückweg (Self-Use-Receivable) am Gerät — VERIFIED (2026-09-04):** `POST /payments/invoices` (990 sat, `order_ref return-test-20260904-3`, `expiry_seconds=3600`) → Operator zahlt aus Wallet of Satoshi → KAI-Invoice-Status `settled=true, amount_paid=990, settled_at 08:24:38Z` → Reconciler-Lauf 08:25:26Z schreibt `receivable_settled` (`evidence_source=rail_lookup`, `order_ref` gebunden), Status `ok`, 0 Waisen. Zwei Vorläufer-Invoices (1.000 sat / 5-min-TTL, 1.000 sat / 1 h) blieben unbezahlt, weil die Gegen-Wallet die Routing-Gebühr nicht mehr decken konnte; eine 950-sat-Invoice läuft ungenutzt ab. Befunde daraus in PR #865 gefixt: die Invoice-Antwort trug keine `payment_request` (bis in `Invoice`/`LightningRail` fehlend), Default-TTL 300 s → 3.600 s; die periodischen 1-sat-Invoices `kai-pay: KAI receive` stammen aus der externen LNbits-Instanz (Lightning-Address), nicht aus dem Repo.
...
**Zweiter Deploy-Vorfall (2026-09-04, `3ff16bf1` mit Fix #865):** `DEPLOY_FAILED/RESTART_FAILED` — der Operator-Unit-Apply von 08:06 UTC hatte für die fünf Dauerläufer bereits die Immutable-Release-Units der Mainline installiert (`WorkingDirectory=/home/kai/current`, `runtime-exec`), obwohl der Release-Pfad noch nicht existiert (Cutover offen); `kai-server` scheiterte mit `200/CHDIR`, `agent-worker`/`entry-watch` als Abhängige (~10 min). Restore der fünf Units aus `/var/backups/kai-units/20260904T080654Z` durch den Operator → alle Dienste aktiv, `/health` ok auf `3ff16bf1`. Befund (nicht Payment-Fabric, sondern Deploy-Kette): `pi_apply_systemd_units.sh` installiert Release-Units vor Release-Existenz; der Installer-Guard aus #855 greift dort nicht. Fix #865 am Gerät verifiziert: `POST /payments/invoices` liefert `payment_request`.

**Value-OS v0.2 (PR #871, Mainline `5dd1e3d8`, deployt 2026-09-04 11:02 UTC) — VERIFIED am Gerät:** (1) Intents überleben Neustarts: `AWAITING_APPROVAL`-Intent `pi_1681d97a22644e73` → `artifacts/payments/intent_vault.jsonl` (AES-256-GCM, 1 Zeile, 0600, keine Klartext-BOLT11) → `kai-server`-Restart → `GET` weiterhin `AWAITING_APPROVAL`, `simulate` liefert Quote (Fee 3 sat); neue Startbedingung `APP_PAYMENT_VAULT_KEY` auf dem Pi gesetzt (0600, nie ausgegeben). (2) Invoice-`memo` erreicht den Node: neueste Invoice trägt `kai-pay: self_test`. (3) Dual-Journal-Abgleich (Altpfad `ln_ops_ledger_v2` ↔ `payment_journal`) lief im Reconcile-Timer mit `dual_conflicts: []`. (4) `get()` liest journal-first (Test); Runbook §3 HOTP-Provisionierung + `scripts/hotp_check_counter.py`. Test: Payment-Suite 676 passed; CI 9/9. Deploy-Guard PR #869 (`5b60efe3`) am Gerät: Dry-Run überspringt die fünf release-gebundenen Units mit `SKIPPED_RELEASE_NOT_ACTIVE`, HOLD statt Installation.
...
1. Runbook um HOTP-Provisionierung (YubiKey/Authenticator, lokaler Zähler-Check) und um den Hinweis »Intent überlebt keinen Restart« ergänzen; Rückweg ist real bewiesen (990 sat, `receivable_settled`).
