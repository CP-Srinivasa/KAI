...
HEADLINES = [
    (
        "morgan stanleys msbt ends first trading month with 0 outflows amid bitcoin etfs 6 week inflow streak",
        "expect_pass",
    ),
...
print()
recovery_rate = (
    stats["expect_pass_OK"] / (stats["expect_pass_OK"] + stats["expect_pass_FAIL"]) * 100
    if (stats["expect_pass_OK"] + stats["expect_pass_FAIL"])
    else 0
)
fp_rate = (
    stats["expect_block_FAIL"] / (stats["expect_block_OK"] + stats["expect_block_FAIL"]) * 100
    if (stats["expect_block_OK"] + stats["expect_block_FAIL"])
    else 0
)
