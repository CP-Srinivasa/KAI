...
| P46 Paper Portfolio Read Surface & Exposure Summary | 🔵 Sprint 40 (Definition abgeschlossen) | Kanonischer Portfolio-Read-Surface-Contract definiert. `PositionSnapshot` (frozen: position_id, symbol, side, quantity, entry_price, current_price, unrealized_pnl_usd, position_value_usd, stop_loss, take_profit, opened_at, as_of, market_data_source, is_mark_to_market). `PaperPortfolioSnapshot` (frozen: as_of, initial_equity, cash, cash_pct, positions-tuple, total_equity_usd, realized_pnl_usd, unrealized_pnl_usd, total_fees_usd, trade_count, is_mark_to_market). `ExposureSummary` (frozen: total_exposure_usd, cash_pct, position_count, largest_position). Kanonische Source: artifacts/paper_execution_audit.jsonl (Fill-Replay). MtM optional, fail-closed per Position. Neue MCP-Tools: get_paper_portfolio_snapshot + get_portfolio_exposure_summary (beide canonical_read). Neue CLI: research paper-portfolio-snapshot + research portfolio-exposure. Telegram: /positions → get_paper_portfolio_snapshot (ersetzt Handoff-Proxy), /exposure → get_portfolio_exposure_summary (ersetzt Stub), "exposure" in _READ_ONLY_COMMANDS. Neues Modul: app/research/portfolio_surface.py. §51 contracts.md, I-291–I-300 intelligence_architecture.md, A-040–A-044 ASSUMPTIONS.md. Implementierungs-Tasks 40.1–40.7 in TASKLIST.md (Codex). |

**Test-Stand**: 1362 Tests passing, ruff clean (2026-03-21) — Vollvalidierung grün: alle Sprints 1-38 ✅
Sprint 39: Definition abgeschlossen — Implementierung pending (Codex)
