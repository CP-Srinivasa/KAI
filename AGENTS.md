...
| P47 TradingLoop Control Plane & Cycle Audit Surface | 🔲 Sprint 41 — Definition abgeschlossen, Implementierung pending (Codex) | Kanonischer Control-Plane-Surface für den vorhandenen TradingLoop. Neues Modul: `app/orchestrator/loop_read.py` — `read_loop_status(audit_path) -> LoopStatus` (synchron, never-raise, JSONL-Replay). Neues Modell: `LoopStatus` (frozen, `app/orchestrator/models.py`): mode, loop_enabled=False, last_cycle_id, last_cycle_status, last_cycle_at_utc, last_cycle_symbol, total_cycles, status_counts, audit_path, generated_at_utc, execution_enabled=False, write_back_allowed=False, live_allowed=False. Read-Only MCP: `get_loop_status` (canonical_read) + `get_loop_cycle_summary` (bestehend). Read-Only CLI: `research loop-status` + `research loop-cycle-summary` (bestehend). Guarded-Write MCP: `run_paper_cycle` (guarded_write) — mode MUSS "paper"|"shadow" sein, "live" fail-closed, MockMarketDataAdapter, fresh PaperExecutionEngine, ephemeres Portfolio (schreibt NICHT in paper_execution_audit.jsonl), appended LoopCycle in trading_loop_audit.jsonl. Guarded-Write CLI: `research run-paper-cycle`. Kein Daemon, kein Scheduler, kein Auto-Retry. §52 contracts.md, I-301–I-310 intelligence_architecture.md, A-047–A-051 ASSUMPTIONS.md. Ziel: ≥ 27 neue Tests, 1453+ gesamt. |

**Test-Stand**: 1426 Tests passing, ruff clean (2026-03-21) — Vollvalidierung grün: alle Sprints 1-40C ✅
Sprint 41: Definition abgeschlossen — Implementierung pending (Codex)
