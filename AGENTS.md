...
| P45 Market Data Layer — Read-Only Adapter Contract | 🔵 Sprint 39 (Definition abgeschlossen) | Kanonischer Market-Data-Contract definiert. `MarketDataPoint` (frozen dataclass: symbol, timestamp_utc, price, volume_24h, change_pct_24h, source, is_stale, freshness_seconds) als einzige kanonische Daten-Einheit fuer TradingLoop. `BaseMarketDataAdapter` ABC: never-raise-contract, read-only-invariante, health_check()-als-Liveness-Signal (kein Routing-Trigger). `MockMarketDataAdapter`: deterministisch (sinusoidal, hash-basierte Phase), kein random(), kein externes Netzwerk — Pflicht-Default fuer Paper-Trading und Tests. Fail-Closed: None/is_stale=True → Zyklus-Ueberspringen (kein Auto-Routing, kein Signal, kein Order). BacktestEngine: pre-fetched dict[str, float] — kein interner Adapter-Aufruf. Adapter-Auswahl: explizite Konfiguration (DI), keine Fallback-Kette. §50 contracts.md (final), I-281–I-290 intelligence_architecture.md, A-032–A-036 ASSUMPTIONS.md. Implementierungs-Tasks 39.1–39.6 in TASKLIST.md (Codex). |
